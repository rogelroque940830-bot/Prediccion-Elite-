...
const LEDGER_SCHEMA_VERSION = "mlb-ledger.v1" as const;
...
const predictionInputSchema = z.object({
  schemaVersion: z.literal(LEDGER_SCHEMA_VERSION).default(LEDGER_SCHEMA_VERSION),
...
export type MlbPredictionInput = z.infer<typeof predictionInputSchema>;
export type MlbSettlementInput = z.infer<typeof settlementInputSchema>;
...
export type LedgerPrediction = ReturnType<typeof mapPredictionRow>;
export type LedgerSettlement = ReturnType<typeof mapSettlementRow>;
...
function computeProfitUnits(result: MlbSettlementInput["result"], stake: number, odds: number): number {
  const winProfit = stake * decimalProfitMultiple(odds);
  if (result === "WIN") return rounded(winProfit, 4);
  if (result === "LOSS") return rounded(-stake, 4);
  if (result === "HALF_WIN") return rounded(winProfit / 2, 4);
  if (result === "HALF_LOSS") return rounded(-stake / 2, 4);
  return 0;
}

function outcomeForScoring(result: string): number | null {
...
function mapPredictionRow(row: PredictionRow) {
  return {
    id: row.id,
...
function mapSettlementRow(row: SettlementRow) {
  return {
    ...
    finalScore: row.final_home_score == null || row.final_away_score == null
      ? null
...
export class MlbLedgerStore {
  ...
  private migrate(): void {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_mlb_settlement_prediction ON mlb_settlement_events_v1(prediction_id, settled_at_ms DESC, recorded_at_ms DESC);

      CREATE TRIGGER IF NOT EXISTS mlb_prediction_immutable_update
      BEFORE UPDATE ON mlb_prediction_ledger_v1
      BEGIN SELECT RAISE(ABORT, 'mlb prediction ledger is immutable'); END;

      CREATE TRIGGER IF NOT EXISTS mlb_prediction_immutable_delete
      BEFORE DELETE ON mlb_prediction_ledger_v1
      BEGIN SELECT RAISE(ABORT, 'mlb prediction ledger is immutable'); END;

      CREATE TRIGGER IF NOT EXISTS mlb_settlement_immutable_update
      BEFORE UPDATE ON mlb_settlement_events_v1
      BEGIN SELECT RAISE(ABORT, 'mlb settlement ledger is immutable'); END;

      CREATE TRIGGER IF NOT EXISTS mlb_settlement_immutable_delete
      BEFORE DELETE ON mlb_settlement_events_v1
      BEGIN SELECT RAISE(ABORT, 'mlb settlement ledger is immutable'); END;
    `);
  ...
  status() {
    const predictions = this.db.prepare("SELECT COUNT(*) AS count FROM mlb_prediction_ledger_v1").get().count as number;
    const settlements = this.db.prepare("SELECT COUNT(*) AS count FROM mlb_settlement_events_v1").get().count as number;
  ...
  appendPrediction(raw: unknown): { data: LedgerPrediction; idempotent: boolean } {
    const parsed = predictionInputSchema.parse(raw);
    ...
    if (parsed.clientRequestId) {
      ...
      if (existing) {
        if (existing.payload_sha256 !== payloadSha) throw Object.assign(new Error("clientRequestId already exists with different payload"), { status: 409 });
        return { data: mapPredictionRow(existing), idempotent: true };
      }
    }

    if (parsed.supersedesId) {
      const superseded = this.db.prepare("SELECT id FROM mlb_prediction_ledger_v1 WHERE id = ?").get(parsed.supersedesId);
      if (!superseded) throw Object.assign(new Error("supersedesId does not exist"), { status: 400 });
    }
    ...
    const comparisonProbability = parsed.probabilities.noVig ?? marketImplied;
    const edgePp = parsed.probabilities.edgePp ?? (parsed.probabilities.model - comparisonProbability) * 100;
  ...
  appendSettlement(predictionId: string, raw: unknown): { data: LedgerSettlement; idempotent: boolean } {
    ...
    const predictionRow = this.db.prepare("SELECT * FROM mlb_prediction_ledger_v1 WHERE id = ?").get(predictionId) as PredictionRow | undefined;
    if (!predictionRow) throw Object.assign(new Error("Prediction not found"), { status: 404 });

    const parsed = settlementInputSchema.parse(raw);
    const payloadJson = canonicalJson(parsed);
    const payloadSha = sha256(payloadJson);
    ...
    if (parsed.clientRequestId) {
      ...
      if (existing) {
        if (existing.payload_sha256 !== payloadSha || existing.prediction_id !== predictionId) {
          throw Object.assign(new Error("clientRequestId already exists with different settlement"), { status: 409 });
        }
        return { data: mapSettlementRow(existing), idempotent: true };
      }
    }
    ...
    const clvPp = closingImplied == null ? null : (closingImplied - predictionRow.market_implied_prob) * 100;
    const profitUnits = parsed.profitUnitsOverride ?? computeProfitUnits(parsed.result, predictionRow.stake_units, predictionRow.odds_american);
    ...
    this.db.prepare(`
      INSERT INTO mlb_settlement_events_v1 (
        ...
        @correctionOf, @notes, @payloadSha, @payloadJson
      )
    `).run({
      eventId,
      predictionId,
    ...
    const row = this.db.prepare("SELECT * FROM mlb_settlement_events_v1 WHERE event_id = ?").get(eventId) as SettlementRow;
    return { data: mapSettlementRow(row), idempotent: false };
  }
  ...
  getPredictionRecord(id: string): ReportRecord | null {
  ...
  latestSettlement(predictionId: string): LedgerSettlement | null {
    const row = this.db.prepare(`
      ...
      WHERE prediction_id = ?
      ORDER BY settled_at_ms DESC, recorded_at_ms DESC, event_id DESC
      LIMIT 1
  ...
  listRecords(filters: {
    ...
    const rows = this.db.prepare(`
      ...
      LEFT JOIN mlb_settlement_events_v1 s ON s.event_id = (
        ...
        ORDER BY s2.settled_at_ms DESC, s2.recorded_at_ms DESC, s2.event_id DESC
        LIMIT 1
      )
      ${where}
    ...
    const records = rows.map((row) => {
      const prediction = mapPredictionRow(row);
      let settlement: LedgerSettlement | null = null;
      if (row.s_event_id) {
...
function summarize(records: ReportRecord[]) {
  const settled = records.filter((record) => record.settlement);
  const graded = settled.filter((record) => outcomeForScoring(record.settlement!.result) != null);
  let weightedWins = 0;
  ...
  for (const record of settled) {
    ...
    if (result !== "VOID") risked += record.prediction.decision.stakeUnits;
    profit += record.settlement!.profitUnits;
    if (record.settlement!.clvPp != null) {
      clvSum += record.settlement!.clvPp!;
      clvCount++;
    }
  }

  for (const record of graded) {
    const y = outcomeForScoring(record.settlement!.result)!;
    if (record.settlement!.result === "WIN") weightedWins += 1;
    else if (record.settlement!.result === "LOSS") weightedLosses += 1;
    else if (record.settlement!.result === "HALF_WIN") { weightedWins += 0.5; weightedLosses += 0.5; }
    else if (record.settlement!.result === "HALF_LOSS") { weightedWins += 0.5; weightedLosses += 0.5; }
  ...
  const decisionDenominator = weightedWins + weightedLosses;
  return {
    predictions: records.length,
    settled: settled.length,
    pending: records.length - settled.length,
    graded: graded.length,
    weightedWins: rounded(weightedWins, 2),
    weightedLosses: rounded(weightedLosses, 2),
    hitRate: decisionDenominator > 0 ? rounded(weightedWins / decisionDenominator, 4) : null,
    unitsRisked: rounded(risked, 4),
    profitUnits: rounded(profit, 4),
    roi: risked > 0 ? rounded(profit / risked, 4) : null,
    averageModelProbability: graded.length ? rounded(modelSum / graded.length, 4) : null,
    averageEdgePp: graded.length ? rounded(edgeSum / graded.length, 3) : null,
    averageClvPp: clvCount ? rounded(clvSum / clvCount, 3) : null,
    brierScore: graded.length ? rounded(brier / graded.length, 6) : null,
    logLoss: graded.length ? rounded(logLoss / graded.length, 6) : null,
  };
}

function groupReport(records: ReportRecord[], selector: (record: ReportRecord) => string) {
...
function calibrationBins(records: ReportRecord[]) {
  const bins = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00];
  return bins.slice(0, -1).map((lower, index) => {
    const upper = bins[index + 1];
    const bucket = records.filter((record) => {
      const probability = record.prediction.probabilities.model;
      return probability >= lower && (index === bins.length - 2 ? probability <= upper : probability < upper) && outcomeForScoring(record.settlement?.result || "") != null;
    });
    const actual = bucket.map((record) => outcomeForScoring(record.settlement!.result)!).reduce((sum, value) => sum + value, 0);
    const predicted = bucket.reduce((sum, record) => sum + record.prediction.probabilities.model, 0);
    return {
      range: `${Math.round(lower * 100)}-${Math.round(upper * 100)}%`,
      count: bucket.length,
      averagePredicted: bucket.length ? rounded(predicted / bucket.length, 4) : null,
      actualHitRate: bucket.length ? rounded(actual / bucket.length, 4) : null,
    };
  });
}

export function buildMlbBacktestReport(records: ReportRecord[], trainPct = 70, validationPct = 15) {
  ...
  const fingerprintMaterial = ordered.map((record) => [
    record.prediction.id,
    record.prediction.payloadSha256,
    record.settlement?.eventId || "pending",
    record.settlement?.payloadSha256 || "pending",
  ].join(":" )).join("\n");

  return {
    schemaVersion: LEDGER_SCHEMA_VERSION,
    generatedAt: new Date().toISOString(),
    datasetSha256: sha256(fingerprintMaterial),
    filtersAppliedBeforeReport: true,
    ...
    byMarket: groupReport(ordered, (record) => record.prediction.market.type),
    byConfidence: groupReport(ordered, (record) => record.prediction.decision.confidenceLabel || "UNKNOWN"),
    bySignal: groupReport(ordered, (record) => record.prediction.decision.signal),
    byAnalysisStage: groupReport(ordered, (record) => record.prediction.analysisStage),
    byMonth: groupReport(ordered, (record) => record.prediction.game.gameDate.slice(0, 7)),
    calibration: calibrationBins(ordered),
    methodology: {
      ...
      latestSettlementEventWins: true,
    },
  };
}
...
function recordsToCsv(records: ReportRecord[]): string {
  const headers = [
    "prediction_id", "recorded_at", "game_date", "game_pk", "away_team", "home_team",
    "market", "selection", "line", "odds_american", "book", "model_probability",
    "market_implied_probability", "no_vig_probability", "edge_pp", "signal",
    "confidence_label", "confidence_pct", "stake_units", "analysis_stage", "model_name",
    "model_version", "git_commit", "settlement_event_id", "result", "settled_at",
    "closing_odds_american", "closing_line", "clv_pp", "profit_units",
  ];
...
export function registerMlbLedgerRoutes(app: Express): void {
  const store = new MlbLedgerStore();
  ...
  app.post("/api/mlb/ledger/v1/predictions", (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ success: false, error: "Invalid MLB ledger prediction", details: error.flatten() });
      }
      return res.status(error?.status || 500).json({ success: false, error: error?.message || "Unable to record prediction" });
    }
  });

  app.get("/api/mlb/ledger/v1/predictions", (req, res) => {
    const settledText = parseOptionalText(req.query.settled);
    const records = store.listRecords({
      from: parseOptionalText(req.query.from),
      to: parseOptionalText(req.query.to),
      market: parseOptionalText(req.query.market),
      confidence: parseOptionalText(req.query.confidence),
      signal: parseOptionalText(req.query.signal),
      stage: parseOptionalText(req.query.stage),
  ...
  app.get("/api/mlb/ledger/v1/predictions/:id", (req, res) => {
    const record = store.getPredictionRecord(decodeURIComponent(req.params.id || ""));
  ...
  app.post("/api/mlb/ledger/v1/predictions/:id/settlements", (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ success: false, error: "Invalid MLB settlement", details: error.flatten() });
      }
      return res.status(error?.status || 500).json({ success: false, error: error?.message || "Unable to settle prediction" });
    }
  });
  ...
  app.get("/api/mlb/ledger/v1/report", (req, res) => {
    try {
      const records = store.listRecords({
        ...
        signal: parseOptionalText(req.query.signal),
        stage: parseOptionalText(req.query.stage),
        limit: parseOptionalNumber(req.query.limit) ?? 10_000,
      });
      const trainPct = parseOptionalNumber(req.query.trainPct) ?? 70;
      const validationPct = parseOptionalNumber(req.query.validationPct) ?? 15;
      res.json({ success: true, data: buildMlbBacktestReport(records, trainPct, validationPct) });
    } catch (error: any) {
      res.status(error?.status || 500).json({ success: false, error: error?.message || "Unable to build MLB report" });
    }
  });
  ...
  app.get("/api/mlb/ledger/v1/export", (req, res) => {
    ...
    const format = parseOptionalText(req.query.format) || "jsonl";
    if (format === "csv") {

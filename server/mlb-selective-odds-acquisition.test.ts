...
test("finalist-only acquisition sends one event request with exact thesis-backed keys and timeout signal", async () => {
  ...
  const providerGame = event({ id: "step8-evt-11", home: "New York Mets", away: "Atlanta Braves" });
  const urls: string[] = [];
  const service = new MlbSelectiveOddsAcquisitionService({
    now: () => new Date(BASE_NOW),
  ...
  const result = await service.acquire({ runId: "step8-exact-finalist", discovery: discovery([g1, g2]), maxRunCredits: 4, reserveCredits: 20, apiKey: "secret" });
...
test("budget denial at higher intrinsic rank stops lower-ranked paid work", async () => {
  ...
  const service = new MlbSelectiveOddsAcquisitionService({
    ...
    fetchFn: async () => {
      calls += 1;
      return jsonResponse([
        event({ id: "step8-evt-21", home: "Top Home", away: "Top Away" }),
        event({ id: "step8-evt-22", home: "Lower Home", away: "Lower Away" }),
      ], 100, 10, 0);
    },
  });
  const result = await service.acquire({ runId: "step8-budget-stop", discovery: discovery([top, lower]), maxRunCredits: 2, reserveCredits: 10, apiKey: "secret" });
...
test("equal-distance same-day doubleheader is ambiguous and spends zero paid credits", async () => {
  const target = game({ gamePk: 31, rank: 1, home: "Detroit Tigers", away: "Cleveland Guardians", startTime: "2026-08-10T20:00:00.000Z" });
  const first = event({ id: "step8-dh-a", home: "Detroit Tigers", away: "Cleveland Guardians", commence: "2026-08-10T19:00:00.000Z" });
  const second = event({ id: "step8-dh-b", home: "Detroit Tigers", away: "Cleveland Guardians", commence: "2026-08-10T21:00:00.000Z" });
  ...
  const result = await service.acquire({ runId: "step8-doubleheader-ambiguous", discovery: discovery([target]), maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" });
  assert.equal(calls, 1);
  assert.equal(result.providerCalls.paidEventOdds, 0);
  assert.equal(result.games[0].status, "EVENT_MATCH_AMBIGUOUS");
});

test("one unparseable same-team commence time forces ambiguity", () => {
  const target = game({ gamePk: 32, rank: 1, home: "Detroit Tigers", away: "Cleveland Guardians", startTime: "2026-08-10T19:00:00.000Z" });
  const valid = event({ id: "step8-dh-valid", home: "Detroit Tigers", away: "Cleveland Guardians", commence: "2026-08-10T19:00:00.000Z" });
  const unknown = event({ id: "step8-dh-unknown", home: "Detroit Tigers", away: "Cleveland Guardians", commence: "not-a-date" });
  assert.equal(matchMlbDiscoveryGameToProviderEvent(target, [valid, unknown]).status, "AMBIGUOUS");
});

test("sole team/date event outside bounded start-time window is not accepted", () => {
  const target = game({ gamePk: 33, rank: 1, home: "Detroit Tigers", away: "Cleveland Guardians", startTime: "2026-08-10T17:00:00.000Z" });
  const wrongHalf = event({ id: "step8-dh-wrong-half", home: "Detroit Tigers", away: "Cleveland Guardians", commence: "2026-08-10T23:10:00.000Z" });
  assert.equal(matchMlbDiscoveryGameToProviderEvent(target, [wrongHalf]).status, "AMBIGUOUS");
...
test("fresh per-market cache pays only for newly required keys", async () => {
  ...
  const providerGame = event({ id: "step8-cache-41", home: "Seattle Mariners", away: "Texas Rangers" });
  ...
  await service.acquire({ runId: "step8-cache-first", discovery: firstPlan, maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" });
  nowMs += 30_000;
  const expanded = discovery([game({ gamePk: 41, rank: 1, home: "Seattle Mariners", away: "Texas Rangers", markets: [H2H(), TOTALS()] })]);
  const second = await service.acquire({ runId: "step8-cache-topup", discovery: expanded, maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" });
  ...
  nowMs += 30_000;
  const third = await service.acquire({ runId: "step8-cache-full", discovery: expanded, maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" });
  assert.equal(third.games[0].status, "CACHE_HIT");
  assert.deepEqual(third.games[0].paidMarketKeysRequested, []);
  assert.equal(calls, 5, "three free probes and two paid calls");
});

test("different run IDs targeting same cache miss are serialized and buy once", async () => {
  const providerGame = event({ id: "step8-cross-run-cache-107", home: "Pittsburgh Pirates", away: "Cincinnati Reds" });
  let paidCalls = 0;
  let probeCalls = 0;
  let remaining = 100;
  let used = 10;
  ...
  const [a, b] = await Promise.all([
    serviceA.acquire({ runId: "step8-cross-cache-a", discovery: plan, maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" }),
    serviceB.acquire({ runId: "step8-cross-cache-b", discovery: plan, maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" }),
  ]);
...
test("serialized distinct runs re-probe provider balance and preserve reserve across runs", async () => {
  const eventA = event({ id: "step8-reserve-a", home: "Milwaukee Brewers", away: "Chicago White Sox" });
  const eventB = event({ id: "step8-reserve-b", home: "Washington Nationals", away: "Miami Marlins" });
  let remaining = 13;
  let used = 100;
  let paidCalls = 0;
  const fetchFn = async (input: string | URL | Request): Promise<Response> => {
    ...
    remaining -= markets.length;
    used += markets.length;
    const providerEvent = url.includes("step8-reserve-a") ? eventA : eventB;
  ...
  const [a, b] = await Promise.all([
    serviceA.acquire({ runId: "step8-reserve-run-a", discovery: planA, maxRunCredits: 2, reserveCredits: 10, apiKey: "secret" }),
    serviceB.acquire({ runId: "step8-reserve-run-b", discovery: planB, maxRunCredits: 2, reserveCredits: 10, apiKey: "secret" }),
  ]);
  assert.equal(a.status, "COMPLETED");
  assert.equal(b.status, "BLOCKED");
  assert.equal(b.games[0].status, "BUDGET_DENIED");
...
test("TTL expiry refreshes only on a new explicit run", async () => {
  let nowMs = Date.parse(BASE_NOW);
  const providerGame = event({ id: "step8-ttl-51", home: "Miami Marlins", away: "Philadelphia Phillies" });
  let paidCalls = 0;
  let remaining = 100;
  let used = 30;
  const service = new MlbSelectiveOddsAcquisitionService({
  ...
  await service.acquire({ runId: "step8-ttl-first", discovery: plan, maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" });
  nowMs += MLB_SELECTIVE_ODDS_CACHE_TTL_MS + 1;
  const refreshed = await service.acquire({ runId: "step8-ttl-second", discovery: plan, maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" });
...
test("same runId concurrent and completed replay consume once; changed plan rejects through Promise", async () => {
  const providerGame = event({ id: "step8-idem-61", home: "San Diego Padres", away: "San Francisco Giants" });
  let calls = 0;
  const service = new MlbSelectiveOddsAcquisitionService({
    now: () => new Date(BASE_NOW),
    fetchFn: async (input) => {
  ...
  const input = { runId: "step8-same-run", discovery: plan, maxRunCredits: 3, reserveCredits: 10, apiKey: "secret" };
...
test("paid network failure charges conservative worst case and blocks lower rank", async () => {
  ...
  const service = new MlbSelectiveOddsAcquisitionService({
    ...
    fetchFn: async (input) => {
      ...
      if (String(input).includes("/events/?")) {
        return jsonResponse([
          event({ id: "step8-evt-71", home: "Houston Astros", away: "Los Angeles Angels" }),
          event({ id: "step8-evt-72", home: "Minnesota Twins", away: "Kansas City Royals" }),
        ], 100, 50, 0);
      }
      throw Object.assign(new Error("socket reset"), { code: "ECONNRESET" });
    },
  });
  const result = await service.acquire({ runId: "step8-network-fail", discovery: discovery([top, lower]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" });
  assert.equal(calls, 2);
...
test("absent market is negative-cached and not repurchased inside TTL", async () => {
  let nowMs = Date.parse(BASE_NOW);
  const providerGame = event({ id: "step8-negative-81", home: "Athletics", away: "Los Angeles Dodgers" });
  let paidCalls = 0;
  const service = new MlbSelectiveOddsAcquisitionService({
    now: () => new Date(nowMs),
    fetchFn: async (input) => {
  ...
  const first = await service.acquire({ runId: "step8-negative-first", discovery: plan, maxRunCredits: 2, reserveCredits: 10, apiKey: "secret" });
  assert.equal(first.games[0].quoteMarkets[0]?.availability, "UNAVAILABLE_FROM_PROVIDER");
  nowMs += 60_000;
  const second = await service.acquire({ runId: "step8-negative-second", discovery: plan, maxRunCredits: 2, reserveCredits: 10, apiKey: "secret" });
...
test("reference-only quote is never executable for Market Edge", async () => {
  const providerGame = event({ id: "step8-ref-91", home: "New York Yankees", away: "Boston Red Sox" });
  const service = new MlbSelectiveOddsAcquisitionService({
    now: () => new Date(BASE_NOW),
...
test("mutated paid-key list rejects through Promise before zero-cost probe", async () => {
  ...
  await assert.rejects(
    () => service.acquire({ runId: "step8-mutated-plan", discovery: discovery([invalid]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" }),
    (error: any) => error instanceof MlbSelectiveOddsPlanError && error.code === "PAID_MARKET_KEY_MUTATION",
  );
  assert.equal(calls, 0);
});

test("forging planned and paid lists together still fails Registry-backed validation", async () => {
  ...
  const service = new MlbSelectiveOddsAcquisitionService({ fetchFn: async () => { calls += 1; throw new Error("should not fetch"); } });
  await assert.rejects(
    () => service.acquire({ runId: "step8-forged-registry", discovery: discovery([forged]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" }),
    (error: any) => error instanceof MlbSelectiveOddsPlanError && error.code === "REGISTRY_MARKET_MAPPING_MISMATCH",
  );
  assert.equal(calls, 0);
});

test("forged horizon, thesis intent, and duplicate market identities reject pre-provider", async () => {
  ...
  await assert.rejects(
    () => service.acquire({ runId: "step8-wrong-scope", discovery: discovery([game({ gamePk: 103, rank: 1, markets: [wrongScope] })]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" }),
    (error: any) => error instanceof MlbSelectiveOddsPlanError && error.code === "MARKET_HORIZON_MISMATCH",
  );
  await assert.rejects(
    () => service.acquire({ runId: "step8-wrong-intent", discovery: discovery([game({ gamePk: 104, rank: 1, markets: [wrongIntent] })]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" }),
    (error: any) => error instanceof MlbSelectiveOddsPlanError && error.code === "MARKET_THESIS_INTENT_MISMATCH",
  );
  await assert.rejects(
    () => service.acquire({ runId: "step8-duplicate", discovery: discovery([duplicated]), maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" }),
...
test("all five current paid market mappings pass Registry-backed validation", async () => {
  const providerGame = event({ id: "step8-all-five", home: "Arizona Diamondbacks", away: "Colorado Rockies" });
  ...
  const result = await service.acquire({ runId: "step8-all-five-current", discovery: plan, maxRunCredits: 5, reserveCredits: 10, apiKey: "secret" });

    ...
    (req, res) => {
      ...
      if (!identity) {
        return res.status(401).json({
          success: false,
          error: "Authenticated user session required for MLB PREMIUM no-ULTRA prospective review.",
      ...
      try {
        ...
        if (error instanceof Error && error.message === MLB_PREMIUM_NO_ULTRA_SOURCE_WINDOW_TRUNCATED) {
          return res.status(409).json({
            success: false,
            error: "The upstream economic-review row window is truncated; refusing to evaluate PREMIUM no-ULTRA on an incomplete cohort.",
            code: MLB_PREMIUM_NO_ULTRA_SOURCE_WINDOW_TRUNCATED,
          });
        }
        console.error("P1 PREMIUM no-ULTRA prospective review error:", error);
        return res.status(500).json({
          success: false,
          error: "Unable to build the MLB PREMIUM no-ULTRA prospective review.",

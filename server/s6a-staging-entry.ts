...
import { startMultisportReadinessWorker, type ReadinessSport } from "./multisport-readiness-service";

const s6aReadiness = startMultisportReadinessWorker();

app.get("/health/s6a-readiness", (_req, res) => {
  const status = s6aReadiness.service.status();
...
app.get("/api/multisport/readiness/v1/status", (_req, res) => {
  res.json({ success: true, data: s6aReadiness.service.status() });
});

app.get("/api/multisport/readiness/v1/latest", (_req, res) => {
  const latest = s6aReadiness.service.readLatest();
  if (!latest) {

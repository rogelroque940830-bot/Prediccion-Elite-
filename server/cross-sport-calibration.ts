...
export function validateCalibrationObservation(row: CalibrationObservation): string[] {
  ...
  const cutoff = Date.parse(row.pregameCutoffAt);
...
/**
 * Deterministic one-dimensional Platt scaling. The method is fixed in advance to avoid
 * selecting a calibrator after seeing target-fold outcomes. L2 regularization is applied
 * only to the slope; Newton updates are bounded and converge deterministically.
 */
export function fitPlattCalibrator(rows: CalibrationObservation[], signalFamily: string): PlattCalibrator {
  ...
  const invalid = selected.flatMap((row) => validateCalibrationObservation(row));
  ...
  for (let iteration = 0; iteration < 100; iteration += 1) {
    ...
    for (const row of selected) {
      const x = logit(row.rawSignal);
      const p = sigmoid(intercept + slope * x);
      const error = p - row.outcome;
    ...
    intercept -= Math.max(-2, Math.min(2, d0));
    slope -= Math.max(-2, Math.min(2, d1));
  ...
  return {
    ...
    intercept,
    slope,
    trainingObservations: selected.length,
...
export function walkForwardPlatt(rows: CalibrationObservation[], signalFamily: string): WalkForwardFold[] {
  ...
  for (let index = 1; index < seasons.length; index += 1) {
    ...
    const train = selected.filter((row) => row.season < targetSeason);
    const test = selected.filter((row) => row.season === targetSeason);
    if (train.length < 20 || !test.length) continue;
    const calibrator = fitPlattCalibrator(train, signalFamily);

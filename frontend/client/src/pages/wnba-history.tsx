      ...
      <Card className="border-purple-500/20 bg-purple-500/[0.04]">
        <CardContent className="p-3">
          <div className="flex items-start gap-2 text-xs text-muted-foreground">
            ...
            <p>Los picks de esta lista son decisiones guardadas por el usuario y se liquidan manualmente. La colección científica shadow se muestra aparte y nunca se mezcla con tu ROI; los juegos activos esperan el marcador oficial y no reducen su cobertura de liquidación.</p>
          </div>
          {shadowQuery.data && (
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
              {[
                ["Juegos monitoreados", shadowQuery.data.report.trackedGames ?? shadowQuery.data.report.terminalGames],
                ["Esperando marcador", shadowQuery.data.report.awaitingOfficialFinal ?? 0],
                ["Finales oficiales", shadowQuery.data.report.officialFinalGames ?? shadowQuery.data.report.settled],
      ...
      {picks.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-muted-foreground">No hay picks WNBA guardados. Ve al Predictor WNBA y guarda tus jugadas.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={pick.id} className={`border-l-4 ${pick.result === "W" ? "border-l-green-500" : pick.result === "L" ? "border-l-red-500" : "border-l-amber-500"}`}>
              <CardContent className="p-3">
                ...
                <div className="flex items-center gap-4 mt-2 text-xs">
                  ...
                  {/* Result selector */}
                  {pick.result === "P" && (
                    <Select onValueChange={(val) => dispatch({ type: "UPDATE_WNBA_PICK", payload: { id: pick.id, result: val } })}>
                  ...
                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-muted-foreground hover:text-red-400 ml-auto"
                    onClick={() => dispatch({ type: "DELETE_WNBA_PICK", payload: pick.id })}>

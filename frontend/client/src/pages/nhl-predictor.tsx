...
export default function NHLPredictor() {
  ...
  const { data: nhlData, isLoading: nhlLoading, refetch: refetchNHL } = useQuery<{ success: boolean; games: any[] }>({
  ...
  // ── Home goalie ──────────────────────────────────────────────────────────
  const [homeSavePct, setHomeSavePct] = useState("0.910");
  const [homeGAA, setHomeGAA] = useState("2.80");
  const [homeRecord, setHomeRecord] = useState("25-10-3");
  ...
  const [homeGF, setHomeGF] = useState("3.2");
  const [homeGA, setHomeGA] = useState("2.9");
  const [homePP, setHomePP] = useState("22");
  const [homePK, setHomePK] = useState("80");
  const [homeCorsi, setHomeCorsi] = useState("51");
  const [homeShotsFor, setHomeShotsFor] = useState("32");
  const [homeShotsAgainst, setHomeShotsAgainst] = useState("29");

  // ── Home momentum ────────────────────────────────────────────────────────
  const [homeStreak, setHomeStreak] = useState("0");
  const [homeWinRate10, setHomeWinRate10] = useState("0.6");
  const [homeB2B, setHomeB2B] = useState(false);
  const [homeDaysRest, setHomeDaysRest] = useState("2");

  // ── Away goalie ──────────────────────────────────────────────────────────
  const [awaySavePct, setAwaySavePct] = useState("0.910");
  const [awayGAA, setAwayGAA] = useState("2.80");
  const [awayRecord, setAwayRecord] = useState("20-15-5");
  const [awayRecentGAA, setAwayRecentGAA] = useState("");

  // ── Away team ────────────────────────────────────────────────────────────
  const [awayTeam, setAwayTeam] = useState("");
  const [awayGF, setAwayGF] = useState("3.2");
  const [awayGA, setAwayGA] = useState("2.9");
  const [awayPP, setAwayPP] = useState("22");
  const [awayPK, setAwayPK] = useState("80");
  const [awayCorsi, setAwayCorsi] = useState("49");
  const [awayShotsFor, setAwayShotsFor] = useState("30");
  const [awayShotsAgainst, setAwayShotsAgainst] = useState("31");

  // ── Away momentum ────────────────────────────────────────────────────────
  const [awayStreak, setAwayStreak] = useState("0");
  const [awayWinRate10, setAwayWinRate10] = useState("0.5");

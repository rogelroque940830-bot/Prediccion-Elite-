...
const js = rows.filter(r => r.file.endsWith(".js")).reduce((s,r)=>s+r.bytes,0);
const css = rows.filter(r => r.file.endsWith(".css")).reduce((s,r)=>s+r.bytes,0);
...
for (const row of rows.filter(r => /\.(js|css|html)$/.test(r.file))) {
...
console.log(JSON.stringify({ files: rows.length, bytes: { js, css, total }, budget, warnings, errors }, null, 2));

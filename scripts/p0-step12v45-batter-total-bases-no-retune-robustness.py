...
REPORT_SCHEMA = "courtedge-p0-step12v45-batter-total-bases-no-retune-robustness.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v45-batter-total-bases-no-retune-robustness-contract.v1"
...
def load_v44(path):
    spec = importlib.util.spec_from_file_location("courtedge_v44_batter_total_bases", path)
    if spec is None or spec.loader is None:
        raise SystemExit("V45_V44_MODULE_LOAD_FAILED")
...
def assert_close(label, actual, expected, tolerance):
    if not math.isfinite(float(actual)) or not math.isfinite(float(expected)):
        raise SystemExit(f"V45_PARITY_NONFINITE:{label}:{actual}:{expected}")
    if abs(float(actual) - float(expected)) > tolerance:
        raise SystemExit(
            f"V45_V44_PARITY_MISMATCH:{label}:{actual}:{expected}:tol={tolerance}"
        )
...
def bootstrap_date_means(frame, metric_arrays, replicates, seed, confidence):
    ...
    if len(daily) < 2:
        raise SystemExit("V45_BOOTSTRAP_TOO_FEW_DATES")
...
def main():
    ...
    parser.add_argument("--v44-contract", required=True)
    parser.add_argument("--v44-script", required=True)
    parser.add_argument("--v44-report", required=True)
    parser.add_argument("--out", required=True)
    args = parser.parse_args()

    contract = load(args.contract)
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V45_CONTRACT_SCHEMA_INVALID")
    v44_contract = load(args.v44_contract)
    v44_report = load(args.v44_report)
    if v44_report.get("classification") != contract["parentEvidence"]["v44ClassificationRequired"]:
        raise SystemExit("V45_V44_PARENT_CLASSIFICATION_INVALID")
    if not v44_report.get("candidateRubricPassed"):
        raise SystemExit("V45_V44_PARENT_GATE_NOT_PASSED")

    v44 = load_v44(args.v44_script)
    seasons = [
        v44_contract["dataBoundary"]["modelFitSeason"],
        v44_contract["dataBoundary"]["validationSeason"],
        *v44_contract["dataBoundary"]["retrospectiveEvaluationSeasons"],
    ]
    records, custody = [], {}
    for season in seasons:
        season_rows, season_custody = v44.build_season_rows(
            args.root, args.batter_root, season, v44_contract
        )
        records.extend(season_rows)
        custody[season] = season_custody

    frame = pd.DataFrame.from_records(records)
    features = tuple(v44_contract["features"]["exactly"])
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit("V45_EMPTY_PARTITION")
    ...
    for metric in contract["replayParity"]["requiredMetrics"]:
        assert_close(
            ...
            v44_report["validation2023"][metric],
            tolerance,
        )
        assert_close(
            f"evaluation2024_2026Ytd.{metric}",
            evaluation_metrics[metric],
            v44_report["evaluation2024_2026Ytd"][metric],
            tolerance,
        )
    ...
    y = evaluation_frame["totalBases"].to_numpy(dtype=float)
    ...
    for line in lines:
        ...
        model_probs = np.asarray([v44.over_probability(mu, dispersion, line) for mu in model_mu])
        batter_probs = np.asarray([v44.over_probability(mu, dispersion, line) for mu in batter_mu])
    ...
    report = {
        ...
        "parent": {
            "v44Classification": v44_report["classification"],
            "v44CandidateRubricPassed": v44_report["candidateRubricPassed"],
            "v44WorkflowRunId": contract["parentEvidence"]["v44WorkflowRunId"],
            "v44ArtifactId": contract["parentEvidence"]["v44ArtifactId"],
        },
        "modelBoundary": {
            "unchangedV44ImplementationReplayed": True,

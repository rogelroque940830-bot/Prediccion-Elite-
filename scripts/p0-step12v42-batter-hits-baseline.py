...
REPORT_SCHEMA = "courtedge-p0-step12v42-batter-hits-baseline.v1"
...
def audit_valid(audit):
    return bool(
        audit
        and audit.get("identityOk")
        and audit.get("sourceHistorical")
        and audit.get("pregame")
        and audit.get("probableBothKnown")
    )
...
def parse_starter(raw):
    ...
    for key, source in mapping.items():
        ...
        if not finite(value) or float(value) < 0:
            raise SystemExit(f"V42_INVALID_STARTER_LINE:{pitcher_id}:{source}:{value}")
...
def empty_pitcher_state():
    return {"starts": 0, "bf": 0.0, "h": 0.0, "k": 0.0, "bb": 0.0}
...
def add_batter_line(state, line, recent_window):
    ...
    state["games"] += 1
    state["pa"] += pa
    state["h"] += int(line["hits"])
    state["tb"] += int(line["totalBases"])
    state["bb"] += int(line["baseOnBalls"])
    state["k"] += int(line["strikeOuts"])
    state["recent"].append({"h": int(line["hits"]), "pa": pa})
...
def batter_league_prior(state):
    ...
    return {
        ...
        "tbpa": state["tb"] / state["pa"],
        "bbpa": state["bb"] / state["pa"],
        "kpa": state["k"] / state["pa"],
    }
...
def shrunk_rate(numerator, denominator, league_rate, prior_weight):
    return (float(numerator) + prior_weight * float(league_rate)) / (
        float(denominator) + prior_weight
    )


def shrunk_mean(total, trials, league_mean, prior_trials):
    return (float(total) + prior_trials * float(league_mean)) / (
        float(trials) + prior_trials
    )
...
def batter_features(state, league_prior, prior_pa):
    recent = state["recent"]
    features = {
        "batter_recent10_hits_per_game": (
            float(np.mean([row["h"] for row in recent])) if recent else None
        ),
        "batter_recent10_pa_per_game": (
            float(np.mean([row["pa"] for row in recent])) if recent else None
        ),
    }
    if league_prior is None:
        features.update(
            {
                "batter_hpa_shrunk": None,
                "batter_tbpa_shrunk": None,
                "batter_bbpa_shrunk": None,
                "batter_kpa_shrunk": None,
            }
        )
        return features
    features.update(
        {
            "batter_hpa_shrunk": shrunk_rate(
                state["h"], state["pa"], league_prior["hpa"], prior_pa
            ),
            "batter_tbpa_shrunk": shrunk_rate(
                state["tb"], state["pa"], league_prior["tbpa"], prior_pa
            ),
            "batter_bbpa_shrunk": shrunk_rate(
                state["bb"], state["pa"], league_prior["bbpa"], prior_pa
            ),
            "batter_kpa_shrunk": shrunk_rate(
                state["k"], state["pa"], league_prior["kpa"], prior_pa
            ),
        }
    )
    return features
...
def lineup_slot_pa_feature(slot_state, global_slot_state, prior_starts):
    ...
    return shrunk_mean(
        slot_state["pa"], slot_state["starts"], league_mean, prior_starts
    )
...
def opposing_starter_features(state, league_prior, rate_prior_bf, workload_prior_starts):
    if league_prior is None:
        return {
            "opposing_starter_hbf_shrunk": None,
            "opposing_starter_kbf_shrunk": None,
            "opposing_starter_bbbf_shrunk": None,
            "opposing_starter_bf_per_start_shrunk": None,
        }
    return {
        "opposing_starter_hbf_shrunk": shrunk_rate(
            state["h"], state["bf"], league_prior["hbf"], rate_prior_bf
        ),
        "opposing_starter_kbf_shrunk": shrunk_rate(
            state["k"], state["bf"], league_prior["kbf"], rate_prior_bf
        ),
        "opposing_starter_bbbf_shrunk": shrunk_rate(
            state["bb"], state["bf"], league_prior["bbbf"], rate_prior_bf
        ),
        "opposing_starter_bf_per_start_shrunk": shrunk_mean(
            state["bf"],
            state["starts"],
            league_prior["bfPerStart"],
...
def nb2_dispersion(y, mu):
    ...
    if denominator <= 0:
        raise SystemExit("V42_NB2_DENOMINATOR_INVALID")
...
def build_season_rows(root, batter_root, season, contract):
    ...
    if canonical.get("schemaVersion") != BASE_SCHEMA:
        raise SystemExit(f"V42_BASE_SCHEMA_INVALID:{season}")
    if batter_history.get("schemaVersion") != "courtedge-mlb-historical-batter-history.v1":
        raise SystemExit(f"V42_BATTER_SCHEMA_INVALID:{season}")
    ...
    for official_date in sorted(by_date):
        ...
        for raw in games:
            ...
            if (
                home_probable is None
                or away_probable is None
                or home_lineup is None
                or away_lineup is None
                or starter_game is None
                or batter_game is None
            ):
                continue
            home_starter = parse_starter(starter_game.get("homeStarter"))
            away_starter = parse_starter(starter_game.get("awayStarter"))
            if (
                home_starter is None
                or away_starter is None
                or home_starter["pitcherId"] != home_probable
                or away_starter["pitcherId"] != away_probable
            ):
                continue
            ...
            for side, lineup, opposing_pitcher_id in (
                ("home", home_lineup, away_probable),
                ("away", away_lineup, home_probable),
            ):
                for lineup_slot, batter_id in enumerate(lineup, 1):
                    ...
                    features = batter_features(
                        batter_state[batter_id],
                        batter_prior,
                        float(cfg["batterRateShrinkagePriorPlateAppearances"]),
                    )
                    slot_pa = lineup_slot_pa_feature(
                        lineup_slot_state[lineup_slot],
                        global_lineup_slot_state,
                        float(cfg["lineupSlotWorkloadShrinkagePriorStarts"]),
                    )
                    features["lineup_slot"] = float(lineup_slot)
                    features["lineup_slot_pa_per_start_shrunk"] = slot_pa
                    features.update(
                        opposing_starter_features(
                            pitcher_state[opposing_pitcher_id],
                            pitcher_prior,
                            float(cfg["pitcherRateShrinkagePriorBattersFaced"]),
                            float(cfg["pitcherWorkloadShrinkagePriorStarts"]),
                        )
                    )
                    batter_only_mu = (
                        float(features["batter_hpa_shrunk"]) * float(slot_pa)
                        if finite(features["batter_hpa_shrunk"]) and finite(slot_pa)
                        else None
                    )
                    rows.append(
                        {
        ...
        # Strict date boundary: score every game on the date before updating any state.
        for raw in games:
            game_pk = int(raw["gamePk"])
            batter_game = batter_game_map.get(game_pk)
            starter_game = starter_map.get(game_pk)
            ...
            if batter_game is not None:
                for side_key in ("homeBatters", "awayBatters"):
                    for line in batter_game[side_key]:
                        ...
                        add_batter_line(
                            batter_state[batter_id],
                            line,
                            int(cfg["recentBatterGamesWindow"]),
                        )
                        add_batter_line(
                            league_batter_state,
                            line,
                ...
                if home_lineup is not None and away_lineup is not None:
                    for side, lineup in (("home", home_lineup), ("away", away_lineup)):
                        ...
                        side_lines = {
                            int(line["batterId"]): line for line in batter_game[side_key]
                        }
                        for lineup_slot, batter_id in enumerate(lineup, 1):
    ...
    return rows, {
        ...
        "eligibilityShareOfFrozenStartingSlots": (
            eligible_positive_pa / frozen_slots if frozen_slots else 0.0
        ),
    }
...
def evaluate(frame, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines):
    ...
    for line in lines:
        ...
        model_probs = np.asarray(
            [over_probability(mu, dispersion, line) for mu in model_mu]
        )
        batter_probs = np.asarray(
            [over_probability(mu, dispersion, line) for mu in batter_mu]
        )
    ...
    return {
        "rows": int(len(frame)),
        "observedMeanHits": observed_mean,
        "meanModelHits": model_mean,
        "meanBatterOnlyHits": batter_mean,
        "trainingConstantMeanHits": float(train_mean),
        ...
        "modelVsConstantAverageBrierImprovement": float(
            np.mean(constant_briers) - np.mean(model_briers)
        ),
        "modelVsBatterOnlyAverageBrierImprovement": float(
            np.mean(batter_briers) - np.mean(model_briers)
        ),
    }
...
def main():
    ...
    if contract.get("schemaVersion") != "courtedge-p0-step12v42-batter-hits-baseline-contract.v1":
        raise SystemExit("V42_CONTRACT_SCHEMA_INVALID")
    ...
    for season in seasons:
        season_rows, season_custody = build_season_rows(
            args.root, args.batter_root, season, contract
        )
    ...
    checks = {
        "validationDevianceBeatsConstant": validation_metrics[
            "modelVsConstantDevianceImprovement"
        ]
        > 0,
        "validationDevianceBeatsBatterOnly": validation_metrics[
            "modelVsBatterOnlyDevianceImprovement"
        ]
        > 0,
        "validationBrierBeatsConstant": validation_metrics[
            "modelVsConstantAverageBrierImprovement"
        ]
        > 0,
        "validationBrierBeatsBatterOnly": validation_metrics[
            "modelVsBatterOnlyAverageBrierImprovement"
        ]
        > 0,
        "evaluationDevianceBeatsConstant": evaluation_metrics[
            "modelVsConstantDevianceImprovement"
        ]
        > 0,
        "evaluationDevianceBeatsBatterOnly": evaluation_metrics[
            "modelVsBatterOnlyDevianceImprovement"
        ]
        > 0,
        "evaluationBrierBeatsConstant": evaluation_metrics[
            "modelVsConstantAverageBrierImprovement"
        ]
        > 0,
        "evaluationBrierBeatsBatterOnly": evaluation_metrics[
            "modelVsBatterOnlyAverageBrierImprovement"
        ]
        > 0,
    }
    ...
    classification = (
        "BATTER_HITS_NO_RETUNE_ROBUSTNESS_CANDIDATE_ONLY"
        if passed
        else "BATTER_HITS_BASELINE_REJECTED"
    )
    counts = Counter(frame["officialDate"])
    frozen_slots = sum(
        row["frozenPregameStartingBatterSlots"] for row in custody.values()
    )
    ...
    report = {
        ...
        "parent": {
            ...
            "v41FiveSeasonCustodyDigest": contract["parentEvidence"][
                "v41FiveSeasonCustodyDigest"
            ],
        },
        "data": {
        ...
        "model": {
            ...
            "coefficientsDescriptiveNotSelectionInput": {
                features[index]: float(model.coef_[index])
                for index in range(len(features))
            },
        },
        "baselines": {
            "constantMeanHits": train_mean,
            "batterOnlyFormula": contract["baselines"]["batterOnlyMechanistic"],
            "sharedTrainingOnlyNb2Dispersion": float(dispersion),
            "constantFixedLineOverProbabilities": {
                str(key): value for key, value in line_probs.items()
            },
        },
        ...
        "volumeDiagnostics": {
            ...
            "medianEligibleBatterOutcomesPerSlateDay": float(
        ...
        "marketBoundary": {
            ...
            "historicalBatterHitsPricesUsed": False,

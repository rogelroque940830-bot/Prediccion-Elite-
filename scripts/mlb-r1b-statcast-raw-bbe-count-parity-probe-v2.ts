...
const SCHEMA = "courtedge-mlb-r1b-statcast-raw-bbe-count-parity-probe.v2" as const;
const UA = "Mozilla/5.0 (compatible; CourtEdge-MLB-R1B-Raw-BBE-V2/1.0)";
...
function rawBbeUrl(season: number, gt: string, lt: string): string {
  return queryUrl("https://baseballsavant.mlb.com/statcast_search/csv", {
    ...
    hfSea: `${season}|`,
    player_type: "pitcher",
    // Savant flag values use two literal dots, and EACH dot must be escaped.
    // v1 incorrectly sent is\..hit\..into\..play| (only the first dot escaped).
    hfFlag: BBE_FLAG,
...
function stat() {
  return { n: 0, exact: 0, diffs: [] as number[] };
}
...
async function main() {
  ...
  // Overlap makes boundary semantics harmless; canonical terminal-pitch identity is deduped.
  const windows = [
  ...
  const [leaderboardText, ...rawTexts] = await Promise.all([
    fetchText(leaderboardUrl),
    ...urls.map(fetchText),
  ]);
    ...
    if (
      ...
      mismatches.push({
        ...
        leaderboard: {
          ...
          ev95plus: expectedEv95plus,
          ev95percent: expectedEv95pct,
          barrels: expectedBarrels,
        },
  ...
  const filterAccepted = nonHitIntoPlayDescriptions === 0
    && chunkDiagnostics.every((chunk) => !chunk.hitCsvSafetyCap);
  ...
  const evidence = {
    ...
    status: "RAW_BBE_FILTER_CORRECTED_PROBE_ONLY_NOT_PARITY_CERTIFICATION",
    ...
    rawCustody: {
      ...
      everyChunkBelowCsvSafetyCap: chunkDiagnostics.every((chunk) => !chunk.hitCsvSafetyCap),
    },
    identity: {
  ...
  const outPath = path.resolve(outArg
    ...
    : "artifacts/mlb-r1b-statcast-raw-bbe-count-parity-probe-v2/evidence.json");
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(evidence, null, 2) + "\n");
  console.log(JSON.stringify({
    filterContract: evidence.filterContract,
    rawCustody: evidence.rawCustody,

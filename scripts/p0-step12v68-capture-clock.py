#!/usr/bin/env python3
"""V68 per-game capture clock using the V80 T-10/T-7/T-4 orchestration pattern.

Operational infrastructure only. This module never reads MLB outcomes or prices.
V68 scientific eligibility remains unchanged: capture strictly before start and
inside the already-frozen <=20 minute window, exact 9x9 lineups, both probable
pitchers, prior-date-only state and immutable first canonical evidence.

Important V68-specific difference from V80: pregame state precedence follows
the certified V68/MLB rule from PR #682, so codedGameState=P can remain pregame
when abstractGameState=Live (for example Warmup).
"""
...
PLAN_SCHEMA = "courtedge-p0-step12v68-capture-clock-plan.v1"
SLATE_SCHEMA = "courtedge-p0-step12v68-daily-slate.v1"
WAIT_SCHEMA = "courtedge-p0-step12v68-capture-clock-wait.v1"
ATTEMPT_LEADS = (10, 7, 4)
DEFAULT_DISPATCH_MIN_LEAD = 4.0
DEFAULT_DISPATCH_MAX_LEAD = 35.0
DEFAULT_STAGE_LATE_TOLERANCE_MINUTES = 0.75
USER_AGENT = "CourtEdge-V68-Capture-Clock/3.0"
...
def fetch_live_game(game_pk: int) -> dict[str, Any]:
    ...
    return {
        ...
        "teams": {
            "home": {
                "team": {
                    "id": int(((gd.get("teams") or {}).get("home") or {}).get("id") or 0)
                }
            },
            "away": {
                "team": {
                    "id": int(((gd.get("teams") or {}).get("away") or {}).get("id") or 0)
                }
            },
        },
    }
    ...
    return {
        ...
        "teams": {
            "home": {
                "team": {
                    "id": int(
                        (((game.get("teams") or {}).get("home") or {}).get("team") or {}).get("id")
                        or 0
                    )
                }
            },
            "away": {
                "team": {
                    "id": int(
                        (((game.get("teams") or {}).get("away") or {}).get("team") or {}).get("id")
                        or 0
                    )
                }
            },
...
def game_disposition(
    game: dict[str, Any], target_date: str, observed_at: dt.datetime
) -> str:
    """Operational disposition with certified V68 pregame-state precedence."""
    status = game.get("status") or {}
...
def attempt_state(directory: str | None) -> tuple[set[int], dict[int, dict[str, Any]]]:
    ...
    for path in Path(directory).glob("*.json"):
        ...
        if current is None or stamp >= str(
            current.get("updatedAt") or current.get("lastObservedAt") or ""
        ):
            retries[gp] = payload
    return terminal, retries
...
def snapshot_payload(
    schedule: Any, target_date: str, observed_at: dt.datetime
) -> dict[str, Any]:
    games = []
...
def snapshot(args: argparse.Namespace) -> None:
    ...
    if operational_window(args.target_date, observed_at) == "BEFORE_WINDOW":
        raise SystemExit("V68_CAPTURE_CLOCK_DAILY_SLATE_BEFORE_1030_ET")
...
def plan(args: argparse.Namespace) -> None:
    ...
    if (
        slate.get("schemaVersion") != SLATE_SCHEMA
        or slate.get("targetOfficialDate") != args.target_date
    ):
        raise SystemExit("V68_CAPTURE_CLOCK_SLATE_SNAPSHOT_INVALID")
...
def locate_game(
    schedule: Any, target_date: str, game_pk: int
) -> dict[str, Any] | None:
    for game in schedule_games(schedule):
...
def wait_stage(args: argparse.Namespace) -> None:
    ...
    if stage not in ATTEMPT_LEADS:
        raise SystemExit(f"V68_CAPTURE_CLOCK_INVALID_STAGE:{stage}")
    ...
    while True:
        ...
        common = {
            ...
            "codedGameState": str((game.get("status") or {}).get("codedGameState") or ""),
            "abstractGameState": str((game.get("status") or {}).get("abstractGameState") or ""),
            ...
            "scheduledStartChanged": bool(
                ...
                and abs(
                    (
                        start
                        - parse_time(args.scheduled_start_from_planner)
                    ).total_seconds()
                )
                >= 30
            ),
        ...
        if disposition in NONTERMINAL_HOLD_DISPOSITIONS:
            if observed_at >= deadline or args.schedule_fixture:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--min-lead-minutes",
        type=float,
        default=DEFAULT_DISPATCH_MIN_LEAD,
    )
    p.add_argument(
        "--max-lead-minutes",
        type=float,
        default=DEFAULT_DISPATCH_MAX_LEAD,
    )
    ...
    w.add_argument(
        ...
        type=float,
        default=DEFAULT_STAGE_LATE_TOLERANCE_MINUTES,
    )

import type { C4PriorLineupSnapshot, C4PriorTeamGame } from "../server/mlb-c4-live-feature-builder";
import type { MlbFull13PriorPitcherLine } from "../server/mlb-full13-live-feature-builder";
...
export function pitcherLine(raw: Json): MlbFull13PriorPitcherLine {
  const earnedRuns = Number(raw.earnedRuns);
  const homeRuns = Number(raw.homeRuns);
  return {
    ...
    earnedRuns: Number.isFinite(earnedRuns) ? earnedRuns : null,
    homeRuns: Number.isFinite(homeRuns) ? homeRuns : null,
  };
}

export interface Full13ReplayState {
  teamHistory: Map<number, C4PriorTeamGame[]>;
  pitcherHistory: Map<number, MlbFull13PriorPitcherLine[]>;
  leagueStarterHistory: MlbFull13PriorPitcherLine[];
  priorLineups: Map<number, C4PriorLineupSnapshot[]>;
}

export function emptyReplayState(): Full13ReplayState {
  return {
    teamHistory: new Map(),
...
export function updateReplayStateForGame(
  ...
  if (auditValid(audit) && lineup?.complete) {
    mapPush(state.priorLineups, homeTeamId, {
      officialDate,
      gamePk,
      battingOrder: lineup.homeBattingOrder.map(Number),
    });
    mapPush(state.priorLineups, awayTeamId, {
      officialDate,
      gamePk,
      battingOrder: lineup.awayBattingOrder.map(Number),
    });
  }
}

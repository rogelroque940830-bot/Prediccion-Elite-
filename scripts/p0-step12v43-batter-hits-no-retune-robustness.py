...
REPORT_SCHEMA = "courtedge-p0-step12v43-batter-hits-no-retune-robustness.v1"
...
def load_v42(path):
    spec = importlib.util.spec_from_file_location("courtedge_v42_batter_hits", path)
    if spec is None or spec.loader is None:
        raise SystemExit("V43_V42_MODULE_LOAD_FAILED")
...
def assert_close(label, actual, expected, tolerance):
    if not math.isfinite(float(actual)) or not math.isfinite(float(expected)):
        raise SystemExit(f"V43_PARITY_NONFINITE:{label}:{actual}:{expected}")
    if abs(float(actual) - float(expected)) > tolerance:
        raise SystemExit(
            f"V43_V42_PARITY_MISMATCH:{label}:{actual}:{expected}:tol={tolerance}"
        )
...
def bootstrap_date_means(frame, metric_arrays, replicates, seed, confidence):
    ...
    if len(daily) < 2:
        raise SystemExit("V43_BOOTSTRAP_TOO_FEW_DATES")
...
def main():
    ...
    parser.add_argument("--v42-contract", required=True)
    parser.add_argument("--v42-script", required=True)
    parser.add_argument("--v42-report", required=True)
    parser.add_argument("--out", required=True)
    args = parser.parse_args()

    contract = load(args.contract)
    if contract.get("schemaVersion") != "courtedge-p0-step12v43-batter-hits-no-retune-robustness-contract.v1":
        raise SystemExit("V43_CONTRACT_SCHEMA_INVALID")
    v42_contract = load(args.v42_contract)
    v42_report = load(args.v42_report)
    if v42_report.get("classification") != contract["parentEvidence"]["v42ClassificationRequired"]:
        raise SystemExit("V43_V42_PARENT_CLASSIFICATION_INVALID")
    if not v42_report.get("candidateRubricPassed"):
        raise SystemExit("V43_V42_PARENT_GATE_NOT_PASSED")

    v42 = load_v42(args.v42_script)
    seasons = [
        v42_contract["dataBoundary"]["modelFitSeason"],
        v42_contract["dataBoundary"]["validationSeason"],
        *v42_contract["dataBoundary"]["retrospectiveEvaluationSeasons"],
    ]
    records, custody = [], {}
    for season in seasons:
        season_rows, season_custody = v42.build_season_rows(
            args.root, args.batter_root, season, v42_contract
        )
        records.extend(season_rows)
        custody[season] = season_custody

    frame = pd.DataFrame.from_records(records)
    features = tuple(v42_contract["features"]["exactly"])
    ...
    validation_metrics = v42.evaluate(
        validation,
        features,
        imputer,
        scaler,
    ...
    evaluation_metrics = v42.evaluate(
        evaluation,
        features,
        imputer,
        scaler,
    ...
    for metric in contract["replayParity"]["requiredMetrics"]:
        assert_close(
            f"validation2023.{metric}",
            validation_metrics[metric],
            v42_report["validation2023"][metric],
            tolerance,
        )
        assert_close(
            f"evaluation2024_2026Ytd.{metric}",
            evaluation_metrics[metric],
            v42_report["evaluation2024_2026Ytd"][metric],
            tolerance,
        )
    ...
    for season in contract["seasonStabilityGate"]["partitions"]:
        ...
        season_checks[season] = {
            "modelDevianceBeatsConstant": metrics[
                "modelVsConstantDevianceImprovement"
            ]
            > 0.0,
            "modelDevianceBeatsBatterOnly": metrics[
                "modelVsBatterOnlyDevianceImprovement"
            ]
            > 0.0,
            "modelAverageBrierBeatsConstant": metrics[
                "modelVsConstantAverageBrierImprovement"
            ]
            > 0.0,
            "modelAverageBrierBeatsBatterOnly": metrics[
                "modelVsBatterOnlyAverageBrierImprovement"
            ]
            > 0.0,
            "modelAbsoluteMeanBiasNoWorseThanConstant": metrics[
                "modelAbsoluteMeanCalibrationBias"
            ]
            <= metrics["constantAbsoluteMeanCalibrationBias"],
            "modelAbsoluteMeanBiasNoWorseThanBatterOnly": metrics[
                "modelAbsoluteMeanCalibrationBias"
            ]
            <= metrics["batterOnlyAbsoluteMeanCalibrationBias"],
        }
    ...
    y = evaluation_frame["hits"].to_numpy(dtype=float)
    ...
    for line in lines:
        ...
        model_probs = np.asarray(
            [v42.over_probability(mu, dispersion, line) for mu in model_mu]
        )
        batter_probs = np.asarray(
            [v42.over_probability(mu, dispersion, line) for mu in batter_mu]
        )
    ...
    classification = (
        ...
        if passed
        else contract["classification"]["fail"]
    )
    report = {
        ...
        "parent": {
            "v42Classification": v42_report["classification"],
            "v42CandidateRubricPassed": v42_report["candidateRubricPassed"],
            "v42WorkflowRunId": contract["parentEvidence"]["v42WorkflowRunId"],
            "v42ArtifactId": contract["parentEvidence"]["v42ArtifactId"],
        },
        "modelBoundary": {
            "unchangedV42ImplementationReplayed": True,

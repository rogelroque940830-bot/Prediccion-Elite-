...
const SCHEMA = "courtedge-mlb-r1b-statcast-raw-bbe-count-parity-probe.v1" as const;
const UA = "Mozilla/5.0 (compatible; CourtEdge-MLB-R1B-Raw-BBE/1.0)";
...
function emptyCounts(): Counts { return { bipRows: 0, launchSpeedRows: 0, ev95plus: 0, barrels: 0 }; }
function stat() { return { n: 0, exact: 0, meanAbs: null as number | null, maxAbs: 0, diffs: [] as number[] }; }
...
function finish(s: ReturnType<typeof stat>) {
  return { n: s.n, exact: s.exact, meanAbs: s.diffs.length ? s.diffs.reduce((a, b) => a + b, 0) / s.diffs.length : null, maxAbs: s.maxAbs };
}
...
async function main() {
  ...
  // Overlapping windows intentionally eliminate ambiguity in >/< boundary semantics.
  // Exact terminal-pitch identity is deduplicated after collection.
  const windows = [
    ["2025-03-01", "2025-05-02"],
    ["2025-04-30", "2025-07-02"],
    ["2025-06-30", "2025-09-02"],
    ["2025-08-31", "2025-10-15"],
  ] as const;
  ...
  for (const chunk of parsedChunks) {
    for (const row of chunk.rows) {
      ...
      if (!String(row.description ?? "").includes("hit_into_play")) nonHitIntoPlayDescriptions++;
      unique.set(id, row);
    }
  }
  ...
  for (const row of unique.values()) {
    ...
    const counts = byPitcher.get(pitcher) ?? emptyCounts();
    counts.bipRows++;
  ...
  const ev95 = stat();
  const barrels = stat();
  const pctFromRaw = stat();
  const leaderboardInternalPct = stat();
  ...
  for (const row of leaderboard.rows) {
    ...
    add(ev95, expectedEv95plus, raw.ev95plus);
    add(barrels, expectedBarrels, raw.barrels);
    const rawPct = raw.bipRows > 0 ? round((100 * raw.ev95plus) / raw.bipRows, 1) : null;
    const internalPct = expectedAttempts && expectedEv95plus != null ? round((100 * expectedEv95plus) / expectedAttempts, 1) : null;
    add(pctFromRaw, expectedEv95pct, rawPct);
    add(leaderboardInternalPct, expectedEv95pct, internalPct);
    if (
      expectedAttempts !== raw.bipRows
      || expectedEv95plus !== raw.ev95plus
      || expectedBarrels !== raw.barrels
      || expectedEv95pct !== rawPct
    ) {
      mismatches.push({
        playerId,
  ...
  const chunkDiagnostics = parsedChunks.map((chunk, index) => ({
    ...
    hitGitHubCsvSafetyCap: chunk.rows.length >= 25_000,
    sha256: sha256(rawTexts[index]),
    hasGamePk: chunk.headers.includes("game_pk"),
    hasPitcher: chunk.headers.includes("pitcher"),
    hasLaunchSpeed: chunk.headers.includes("launch_speed"),
    hasLaunchSpeedAngle: chunk.headers.includes("launch_speed_angle"),
  }));

  const comparison = {
    attempts: finish(attempts),
    ev95plus: finish(ev95),
    barrels: finish(barrels),
    hardHitPctFromRawCounts: finish(pctFromRaw),
    leaderboardHardHitInternalFormula: finish(leaderboardInternalPct),
  };
  ...
  const evidence = {
    ...
    status: "RAW_BBE_COUNT_PARITY_PROBE_ONLY_NOT_PARITY_CERTIFICATION",
    ...
    rawCustody: {
      ...
      everyChunkBelowCsvSafetyCap: chunkDiagnostics.every((chunk) => !chunk.hitGitHubCsvSafetyCap),
    },
    ...
    decision: {
      ...
      nextGate: exactCountBridge
        ? "FREEZE_DATE_BOUNDED_CONTACT_QUALITY_ADAPTER; RESOLVE_FIVE_OR_FEWER PROVIDED_EV95PERCENT_EDGE_CASES_IF_PRESENT; THEN XERA_ERA_SCALE"
        : "RESOLVE_RAW_BBE_COUNT_OR_FILTER_SEMANTIC_MISMATCH_BEFORE_XERA_RESEARCH",
    },
  };

  const outArg = process.argv.find((arg) => arg.startsWith("--out="));
  const outPath = path.resolve(outArg ? outArg.slice("--out=".length) : "artifacts/mlb-r1b-statcast-raw-bbe-count-parity-probe/evidence.json");
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(evidence, null, 2) + "\n");
  console.log(JSON.stringify({ rawCustody: evidence.rawCustody, identity: evidence.identity, comparison, exactCountBridge, mismatchCount: mismatches.length, mismatchExamples: mismatches.slice(0, 10) }, null, 2));
}

...
REPORT_SCHEMA = "courtedge-p0-step12v49-batter-runs-scored-no-retune-robustness.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v49-batter-runs-scored-no-retune-robustness-contract.v1"
...
def load_v48(path):
    spec = importlib.util.spec_from_file_location("courtedge_v48_batter_runs_scored", path)
    if spec is None or spec.loader is None:
        raise SystemExit("V49_V48_MODULE_LOAD_FAILED")
...
def assert_close(label, actual, expected, tolerance):
    if not math.isfinite(float(actual)) or not math.isfinite(float(expected)):
        raise SystemExit(f"V49_PARITY_NONFINITE:{label}:{actual}:{expected}")
    if abs(float(actual) - float(expected)) > tolerance:
        raise SystemExit(f"V49_V48_PARITY_MISMATCH:{label}:{actual}:{expected}:tol={tolerance}")
...
def bootstrap_date_means(frame, metric_arrays, replicates, seed, confidence):
    ...
    if len(daily) < 2:
        raise SystemExit("V49_BOOTSTRAP_TOO_FEW_DATES")
...
def main():
    ...
    parser.add_argument("--v48-contract", required=True)
    parser.add_argument("--v48-script", required=True)
    parser.add_argument("--v48-report", required=True)
    parser.add_argument("--out", required=True)
    args = parser.parse_args()

    contract = load(args.contract)
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V49_CONTRACT_SCHEMA_INVALID")
    v48_contract = load(args.v48_contract)
    v48_report = load(args.v48_report)
    if v48_report.get("classification") != contract["parentEvidence"]["v48ClassificationRequired"]:
        raise SystemExit("V49_V48_PARENT_CLASSIFICATION_INVALID")
    if not v48_report.get("candidateRubricPassed"):
        raise SystemExit("V49_V48_PARENT_GATE_NOT_PASSED")

    v48 = load_v48(args.v48_script)
    seasons = [
        v48_contract["dataBoundary"]["modelFitSeason"],
        v48_contract["dataBoundary"]["validationSeason"],
        *v48_contract["dataBoundary"]["retrospectiveEvaluationSeasons"],
    ]
    records, custody = [], {}
    for season in seasons:
        season_rows, season_custody = v48.build_season_rows(args.root, args.batter_root, season, v48_contract)
        records.extend(season_rows)
        custody[season] = season_custody

    frame = pd.DataFrame.from_records(records)
    features = tuple(v48_contract["features"]["exactly"])
    ...
    dispersion = v48.nb2_dispersion(y_train, train_mu)
    ...
    validation_metrics = v48.evaluate(validation, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines)
    evaluation_metrics = v48.evaluate(evaluation, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines)

    tolerance = float(contract["replayParity"]["absoluteTolerance"])
    for metric in contract["replayParity"]["requiredMetrics"]:
        assert_close(f"validation2023.{metric}", validation_metrics[metric], v48_report["validation2023"][metric], tolerance)
        assert_close(f"evaluation2024_2026Ytd.{metric}", evaluation_metrics[metric], v48_report["evaluation2024_2026Ytd"][metric], tolerance)

    season_metrics = {}
    season_checks = {}
    for season in contract["seasonStabilityGate"]["partitions"]:
        metrics = v48.evaluate(
    ...
    y = evaluation_frame["runs"].to_numpy(dtype=float)
    ...
    for line in lines:
        ...
        model_probs = np.asarray([v48.over_probability(mu, dispersion, line) for mu in model_mu])
        batter_probs = np.asarray([v48.over_probability(mu, dispersion, line) for mu in batter_mu])
    ...
    report = {
        ...
        "parent": {
            "v48Classification": v48_report["classification"],
            "v48CandidateRubricPassed": v48_report["candidateRubricPassed"],
            "v48WorkflowRunId": contract["parentEvidence"]["v48WorkflowRunId"],
            "v48ArtifactId": contract["parentEvidence"]["v48ArtifactId"],
        },
        "modelBoundary": {
            "unchangedV48ImplementationReplayed": True,

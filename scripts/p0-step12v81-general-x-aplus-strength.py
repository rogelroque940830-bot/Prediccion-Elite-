...
def relation_filter(rows, rel):
    if rel == "ALL":
        return rows
    if rel == "PEER_OR_WEAKER":
        return [r for r in rows if r["tierRelation"] in ("PEER", "SELECTED_WEAKER")]
    return [r for r in rows if r["tierRelation"] == rel]


def cell(name, rows, filters):
    s = summarize(rows)
    return {"name": name, "filters": filters, **s}
...
def main():
    ...
    for key, p in parent_paths.items():
        ...
        if got != expected:
            raise SystemExit(f"V81_PARENT_BLOB_DRIFT:{key}:{got}")

    v69 = module(a.v69_scorer, "v81_v69")
    v79 = module(a.v79_scorer, "v81_v79")
    v69c = load(a.v69_contract)
    premium_rules_version, premium_rules, models = v69.parse_frozen_classifier_source(a.classifier_source)
    if premium_rules_version != c["immutableParents"]["frozenAPlusClassifier"]["requiredVersion"]:
    ...
    if len(calls) != expected_calls or calls[4]["cap"] != 1:
        raise SystemExit(f"V81_V69_CAPTURE_DRIFT:{len(calls)}")
    ...
    if len(general0) != int(c["targetCohort"]["expectedDecisiveRows"]):
        raise SystemExit(f"V81_GENERAL_COUNT_DRIFT:{len(general0)}")
    if sum(r["y"] == 1 for r in general0) != int(c["targetCohort"]["expectedWins"]):
        raise SystemExit("V81_GENERAL_WIN_COUNT_DRIFT")
    if sum(r["y"] == 0 for r in general0) != int(c["targetCohort"]["expectedLosses"]):
        raise SystemExit("V81_GENERAL_LOSS_COUNT_DRIFT")
    ...
    minimum_games = int(load(a.v79_contract)["strengthDefinition"]["minimumPriorGamesPerTeam"])
    ...
    enriched = []
    exact_aplus_count = 0
    exact_premium_count = 0
    for r in general:
        raw = raw_map[(r["season"], int(r["gamePk"]))]
        features = raw.get("features") or {}
        cls = v69.classify(features, premium_rules, models)
        sign = 1.0 if r["side"] == "HOME" else -1.0
        c4_home = float(cls["aPlusC4PHome"])
        f13_home = float(cls["aPlusFull13PHome"])
        c4_sel = c4_home if sign > 0 else 1.0-c4_home
        f13_sel = f13_home if sign > 0 else 1.0-f13_home
        ...
        rr = dict(r)
        rr.update({
            "aPlusC4Selected": c4_sel,
            "aPlusFull13Selected": f13_sel,
            "aPlusMinSelected": min(c4_sel, f13_sel),
            "aPlusMeanSelected": 0.5*(c4_sel+f13_sel),
            "aPlusFrozenThresholdMargin": min(c4_sel-0.69, f13_sel-0.64),
            "directionalPremiumSupportCount": int(premium_support),
            "directionalC4StrongSupportCount": int(c4_support),
            "rawFrozenPremiumAHome": bool(cls["premiumA"]),
            "rawFrozenAPlusHome": bool(cls["aPlus"]),
            "rawFrozenF5ConsensusHome": bool(cls["f5Consensus"]),
        })
        exact_aplus_count += int(cls["aPlus"])
        exact_premium_count += int(cls["premiumA"])
        enriched.append(rr)
        ...
        for t in score_thresholds:
            for pk in premium_counts:
                for ck in c4_counts:
                    ...
                    tables["jointGrid"].append(cell(
                        f"{rel}|MIN>={t:.2f}|P>={pk}|C4>={ck}", z,
                        {"strengthRelation": rel, "aPlusMinSelectedGe": t, "directionalPremiumSupportCountGe": pk, "directionalC4StrongSupportCountGe": ck}
                    ))

    min_table = int(c["analysisPlan"]["minimumRowsForCandidateTable"])
    min_serious = int(c["analysisPlan"]["minimumRowsForSeriousCandidate"])
    ...
    ranked = sorted(eligible_cells, key=key)
    ranked_serious = sorted(serious, key=key)
    ...
    win_signal = [r["aPlusMinSelected"] for r in enriched if r["y"] == 1]
    loss_signal = [r["aPlusMinSelected"] for r in enriched if r["y"] == 0]
    report = {
        ...
        "signalDistribution": {
            ...
            "medianAPlusMinWins": float(np.median(win_signal)),
            "medianAPlusMinLosses": float(np.median(loss_signal)),
        },
        "predeclaredTables": tables,
        "strongestPredeclaredViews": strongest_predeclared,
        "candidateRanking": {
            "minimumRowsForTable": min_table,
            "minimumRowsForSeriousCandidate": min_serious,
            "top15AllEligible": ranked[:15],
            "top15Serious": ranked_serious[:15],
        },
        "integrity": {
            "thresholdsAddedAfterOutcomeInspection": False,
            "featuresSearchedOutsideFrozenAPlusFamilies": False,

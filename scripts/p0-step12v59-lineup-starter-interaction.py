...
REPORT_SCHEMA='courtedge-p0-step12v59-lineup-starter-interaction.v1'
CONTRACT_SCHEMA='courtedge-p0-step12v59-lineup-starter-interaction-contract.v1'
...
def interaction_name(component,starter_metric):
    short={'lineup_expected_hits':'hits','lineup_expected_total_bases':'total_bases','lineup_expected_walks':'walks','lineup_expected_strikeouts':'strikeouts','lineup_expected_home_runs':'home_runs'}[component]
    return f'matchup_{short}_x_{starter_metric.replace("starter_","")}_diff'

def build_rows(control_root,lineup_root,batter_root,season,c):
    ct=load(os.path.join(control_root,season,'game-anatomy-feature-table.json'))
    lt=load(os.path.join(lineup_root,season,'game-anatomy-feature-table.json'))
    ls=load(os.path.join(lineup_root,season,'cohort','pregame-lineup-history.json'))
    bh=load(os.path.join(batter_root,season,'batter-history.json'))
    if ct.get('schemaVersion')!=BASE_SCHEMA or lt.get('schemaVersion')!=BASE_SCHEMA:raise SystemExit(f'V59_BASE_SCHEMA_INVALID:{season}')
    if bh.get('schemaVersion')!=BATTER_SCHEMA:raise SystemExit(f'V59_BATTER_SCHEMA_INVALID:{season}')
    ...
    if len(cr)!=exp or len(lr)!=exp:raise SystemExit(f'V59_ROW_COUNT_DRIFT:{season}:{len(cr)}:{len(lr)}:{exp}')
    lid={int(r['gamePk']):str(r['officialDate']) for r in lr}
    if len(lid)!=exp:raise SystemExit(f'V59_LINEAGE_DUPLICATE_GAMEPK:{season}')
    for r in cr:
        if lid.get(int(r['gamePk']))!=str(r['officialDate']):raise SystemExit(f"V59_CROSS_SOURCE_IDENTITY_MISMATCH:{season}:{r['gamePk']}")
    ...
    if len(bm)<exp or missing:raise SystemExit(f'V59_BATTER_CUSTODY_INSUFFICIENT:{season}:{len(bm)}:{len(missing)}')
    by=defaultdict(list)
    for r in cr:by[str(r['officialDate'])].append(r)
    bst=defaultdict(empty_batter);league=empty_batter();sst=defaultdict(empty_slot);gst=empty_slot()
    prior_pa=float(c['playerPriorEngineering']['batterRateShrinkagePriorPlateAppearances']);prior_starts=float(c['playerPriorEngineering']['lineupSlotWorkloadShrinkagePriorStarts'])
    base=tuple(c['control']['featuresExactly']);components=tuple(c['playerPriorEngineering']['teamLevelComponentsExactly']);diffs=tuple(c['playerPriorEngineering']['gameLevelMainEffectsExactly']);starter_metrics=tuple(c['opposingStarterProfile']['sideSpecificPregameMetricsExactly']);expected_interactions=tuple(c['interactionLayer']['featuresExactly'])
    out=[];main_avail=0;interaction_avail=0
    for d in sorted(by):
        games=sorted(by[d],key=lambda r:int(r['gamePk']));lp=rates(league)
        for r in games:
            pk=int(r['gamePk']);h,a=lineup(lm.get(pk))
            if h is None or a is None:raise SystemExit(f'V59_COMPLETE_T5_LINEUP_REQUIRED:{season}:{pk}')
            ...
            if result not in ('HOME','AWAY'):raise SystemExit(f'V59_FULL_GAME_BINARY_TARGET_INVALID:{season}:{pk}:{result}')
            ...
            for comp,dn in zip(components,diffs):
                ok=finite(hg[comp]) and finite(ag[comp]);row[dn]=float(hg[comp])-float(ag[comp]) if ok else None;all_main=all_main and ok
            if all_main:main_avail+=1
            all_int=True;generated=[]
            for comp in components:
                for sm in starter_metrics:
                    name=interaction_name(comp,sm);generated.append(name)
                    hs=fv.get('home_'+sm);as_=fv.get('away_'+sm)
                    ok=finite(hg[comp]) and finite(ag[comp]) and finite(hs) and finite(as_)
                    row[name]=(float(hg[comp])*float(as_)-float(ag[comp])*float(hs)) if ok else None
                    all_int=all_int and ok
            if tuple(generated)!=expected_interactions:raise SystemExit('V59_INTERACTION_ORDER_DRIFT')
            if all_int:interaction_avail+=1
            out.append(row)
        for r in games:
            pk=int(r['gamePk']);h,a=lineup(lm.get(pk));bg=bm.get(pk)
            if h is None or a is None or bg is None:raise SystemExit(f'V59_DATE_UPDATE_INPUT_MISSING:{season}:{pk}')
    ...
    return out,{'rows':len(out),'completeT5LineupGames':len(out),'frozenStartingSlots':18*len(out),'gamesWithAllFiveLineupMainEffectsAvailable':main_avail,'gamesWithAllFifteenInteractionFeaturesAvailable':interaction_avail,'gamesUsingTrainingOnlyMedianImputationForAtLeastOneInteraction':len(out)-interaction_avail}

def prep(rows,fs):
    med=[];mean=[];scale=[]
    for f in fs:
        vals=[float(r[f]) for r in rows if finite(r.get(f))]
        if not vals:raise SystemExit(f'V59_TRAIN_FEATURE_EMPTY:{f}')
...
def fit_bin(X,y,l2):
    ...
    if not o.success or not np.all(np.isfinite(o.x)):raise SystemExit(f'V59_BINARY_OPTIMIZER_FAILED:{o.message}')
    return o.x
def cal_bin(raw,y):
    def obj(t):
    ...
    if not o.success or not np.all(np.isfinite(o.x)):raise SystemExit(f'V59_BINARY_CALIBRATION_FAILED:{o.message}')
...
def close(label,a,b,tol):
    if abs(float(a)-float(b))>tol:raise SystemExit(f'V59_V16_METRIC_PARITY_FAILED:{label}:{a}:{b}')
...
def main():
    ...
    c=load(x.contract);vr=load(x.v16_report);vm=load(x.v16_manifest)
    if c.get('schemaVersion')!=CONTRACT_SCHEMA:raise SystemExit('V59_CONTRACT_INVALID')
    if vr.get('schemaVersion')!=V16_REPORT_SCHEMA or vm.get('schemaVersion')!=V16_MANIFEST_SCHEMA:raise SystemExit('V59_V16_SCHEMA_INVALID')
    if vr.get('classification')!=c['parentEvidence']['v16RequiredClassification'] or digest(vm)!=c['parentEvidence']['v16ManifestSha256']:raise SystemExit('V59_V16_PARENT_INVALID')
    ctl=vm['fullGame']
    if ctl.get('featureSet')!=c['control']['featureSetRequired'] or ctl['preprocessor']['features']!=c['control']['featuresExactly']:raise SystemExit('V59_V16_CONTROL_INVALID')
    ...
    base=list(c['control']['featuresExactly']);main=list(c['playerPriorEngineering']['gameLevelMainEffectsExactly']);inter=list(c['interactionLayer']['featuresExactly']);add_fs=tuple(base+main);ch_fs=tuple(base+main+inter)
    if len(ch_fs)!=int(c['challenger']['featureCount']) or len(set(ch_fs))!=len(ch_fs):raise SystemExit('V59_CHALLENGER_FEATURE_COUNT_INVALID')
    additive=fit_model(tr,ca,add_fs,float(c['challenger']['l2Strength']),'V16_C4_PLUS_V58_LINEUP_DIFF5_REPRODUCED')
    challenger=fit_model(tr,ca,ch_fs,float(c['challenger']['l2Strength']),'V16_C4_PLUS_LINEUP_DIFF5_PLUS_LINEUP_X_STARTER15')
    ...
    comps={}
    for s in evs:
        ys=np.asarray([r['homeWin'] for r in rows[s]]);cm=metrics(predict(ctl,rows[s]),ys);am=metrics(predict(additive,rows[s]),ys);nm=metrics(predict(challenger,rows[s]),ys)
        comps[s]={'rows':len(rows[s]),'control':cm,'additiveBenchmark':am,'challenger':nm,'improvementVsControl':compare(cm,nm),'improvementVsAdditiveBenchmark':compare(am,nm)}
    ...
    bcfg=c['pairedBootstrap'];boot=bootstrap(ev,cp,npred,y,int(bcfg['resamples']),int(bcfg['seed']),float(bcfg['confidenceLevel']));rub=c['candidateRubric'];both=[comps[s]['improvementVsControl']['logLoss']>0 and comps[s]['improvementVsControl']['brier']>0 for s in evs];worse=[comps[s]['improvementVsControl']['logLoss']<0 and comps[s]['improvementVsControl']['brier']<0 for s in evs]

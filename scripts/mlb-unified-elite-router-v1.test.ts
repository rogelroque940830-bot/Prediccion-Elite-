...
// A+ is absolute priority in both arms.
{
  const x = base();
...
// Premium is reachable only after A+ NO_PLAY and blocks all lower tiers.
{
  const x = base();
...
// Challenger PP_HORIZON is reachable only after both parent tiers are NO_PLAY.
{
  const x = base();
...
// A regular PP NO_PLAY explicitly forbids Full Modular in the challenger.
{
  const x = base();
...
// Every preregistered technical reason activates the Full Modular safety fallback.
...
// An unregistered technical reason fails closed instead of creating a discretionary fallback.
{
  const x = base();
  x.ppHorizon = {
...
// If the technical fallback itself has no pick, the challenger remains NO_PLAY.
{
  const x = base();
...
// Control is independent of any valid PP state after A+/Premium NO_PLAY.
...
// No valid lower-tier route returns NO_PLAY and never emits more than one pick.
{
...
// Any selection with a different official date is an integrity failure.
{
  const x = base();
  x.fullModular = {
    status: "SELECTION",
...
// Frozen prospective boundary is enforced.
{
  const x = base();
...
// Canonical output strips any unregistered payload field and keeps exactly one identity.
{
  const x = base();
  x.ppHorizon = {
    status: "SELECTION",
...
// Wrapper parity: individual arm helpers equal the combined result.
{
  const x = base();

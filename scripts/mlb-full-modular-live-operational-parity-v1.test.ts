...
function fixture(): FullModularLiveOperationalInput {
  ...
  const homeTeamHistory = priorDates.map((officialDate, i) => ({
    officialDate,
    gamePk: 4100 + i,
    runsFor: 4 + (i % 3),
    runsAgainst: 2 + (i % 2),
  }));
  const awayTeamHistory = priorDates.map((officialDate, i) => ({
    officialDate,
    gamePk: 4200 + i,
    runsFor: 3 + (i % 2),
    runsAgainst: 4 + (i % 3),
  ...
  const leagueStarterHistory = [
    ...homeStarterHistory,
    ...awayStarterHistory,
    ...priorDates.slice(0, 4).map((officialDate, i) => pitcherLine(officialDate, 5300 + i, 303, 25, 6, 2, 2, 1)),
  ];
  return {
    ...
    v39: {
      home: { asOfDate: "2026-08-17", features: v39Features(0.2) },
      away: { asOfDate: "2026-08-17", features: v39Features(-0.2) },
    },
    pitchQualityHistory: priorDates.slice(-4).map((date, i) => pitchQualityRow(date, 6100 + i, 95 + i * 0.1, 93 + i * 0.1)),
    bullpen: {
      homeHistory: [
        { officialDate: "2026-08-15", bullpenPitches: 34, relievers: { "701": 15, "702": 11, "703": 8 } },
        { officialDate: "2026-08-16", bullpenPitches: 29, relievers: { "701": 10, "704": 19 } },
        { officialDate: "2026-08-17", bullpenPitches: 41, relievers: { "701": 16, "702": 14, "705": 11 } },
      ],
      awayHistory: [
        { officialDate: "2026-08-15", bullpenPitches: 38, relievers: { "801": 14, "802": 13, "803": 11 } },
        { officialDate: "2026-08-16", bullpenPitches: 43, relievers: { "801": 18, "804": 25 } },
        { officialDate: "2026-08-17", bullpenPitches: 36, relievers: { "801": 12, "802": 10, "805": 14 } },
      ],
    },
  };
}

function clone(): FullModularLiveOperationalInput {
  return JSON.parse(JSON.stringify(fixture())) as FullModularLiveOperationalInput;
}

function assertNoPlay(input: FullModularLiveOperationalInput, reason: string): void {
  const result = assessFullModularLiveOperationalParity(input);
  assert.equal(result.status, "NO_PLAY");
  if (result.status === "NO_PLAY") assert.equal(result.reason, reason);
}
...
const deterministic = assessFullModularLiveOperationalParity(clone());
assert.deepEqual(deterministic, ready);

{
  const x = clone();
  delete x.v39.home.features.pitcher_prior_starts;
  assertNoPlay(x, "V39_REQUIRED_RAW_FEATURE_MISSING");
}
{
  const x = clone();
  x.v39.home.asOfDate = x.full13.officialDate;
  assertNoPlay(x, "V39_SOURCE_NOT_PRIOR_DATE");
}
{
  const x = clone();
  x.pitchQualityHistory.push(pitchQualityRow(x.full13.officialDate, 6999, 96, 94));
  assertNoPlay(x, "V62_SOURCE_NOT_PRIOR_DATE");
}
{
  const x = clone();
  x.bullpen.awayHistory.push({ officialDate: "2026-08-19", bullpenPitches: 1, relievers: { "899": 1 } });
  assertNoPlay(x, "V66_SOURCE_NOT_PRIOR_DATE");
}
{
  const x = clone();
  x.observedAtUtc = "2026-08-18T22:55:00.001Z";
  assertNoPlay(x, "DECISION_TIMESTAMP_MISSING_OR_LATE");
}
{
  const x = clone();
  x.full13.homeStarterId = null;
  assertNoPlay(x, "PROBABLE_STARTER_UNAVAILABLE");
}
{
  const x = clone();
  x.full13.homeBattingOrder = x.full13.homeBattingOrder?.slice(0, 8) ?? null;
  assertNoPlay(x, "CONFIRMED_LINEUP_UNAVAILABLE");
}
{
  const x = clone();
  x.full13.homeTeamHistory.push({ officialDate: x.full13.officialDate, gamePk: 777001, runsFor: 5, runsAgainst: 1 });
  assertNoPlay(x, "FULL13_PRIOR_KNOWLEDGE_MISSING");
}
{
  const x = clone() as FullModularLiveOperationalInput & { targetOutcome?: string };
  x.targetOutcome = "HOME_WIN";
  assertNoPlay(x, "SOURCE_INTEGRITY_FAILED");
}
{
  const x = clone() as FullModularLiveOperationalInput & { moneyline?: number };
  x.moneyline = -135;
  assertNoPlay(x, "SOURCE_INTEGRITY_FAILED");
}
{
  const x = clone();
  x.pitchQualityHistory = [];
  assertNoPlay(x, "V62_REQUIRED_STARTER_QUALITY_MISSING");
}
{
  const x = clone();
  x.bullpen.homeHistory = [];
  assertNoPlay(x, "V66_REQUIRED_BULLPEN_WORKLOAD_MISSING");
}

...
REPORT_SCHEMA = "courtedge-p0-step12v46-batter-rbis-baseline.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v46-batter-rbis-baseline-contract.v1"
...
def parse_starter(raw):
    ...
    if pitcher_id is None:
        return None
    mapping = {"bf": "battersFaced", "er": "earnedRuns", "bb": "baseOnBalls", "h": "hits"}
    values = {"pitcherId": pitcher_id}
    for key, source in mapping.items():
        value = raw.get(source, 0)
        if not finite(value) or float(value) < 0:
            raise SystemExit(f"V46_INVALID_STARTER_LINE:{pitcher_id}:{source}:{value}")
        values[key] = float(value)
    return values


def empty_batter_state():
    return {"games": 0, "pa": 0.0, "rbi": 0.0, "hr": 0.0, "h": 0.0, "tb": 0.0, "recent": []}


def empty_pitcher_state():
    return {"starts": 0, "bf": 0.0, "er": 0.0, "bb": 0.0, "h": 0.0}
...
def add_batter_line(state, line, recent_window):
    ...
    state["recent"].append({"rbi": rbi, "pa": pa})
...
def batter_features(state, league_prior, prior_pa):
    recent = state["recent"]
    features = {
        "batter_recent10_rbi_per_game": float(np.mean([row["rbi"] for row in recent])) if recent else None,
        "batter_recent10_pa_per_game": float(np.mean([row["pa"] for row in recent])) if recent else None,
    }
    if league_prior is None:
        features.update({"batter_rbipa_shrunk": None, "batter_hrpa_shrunk": None, "batter_hpa_shrunk": None, "batter_tbpa_shrunk": None})
        return features
    features.update({
        "batter_rbipa_shrunk": shrunk_rate(state["rbi"], state["pa"], league_prior["rbipa"], prior_pa),
        ...
        "batter_tbpa_shrunk": shrunk_rate(state["tb"], state["pa"], league_prior["tbpa"], prior_pa),
    })
    return features
...
def opposing_starter_features(state, league_prior, rate_prior_bf, workload_prior_starts):
    if league_prior is None:
        return {"opposing_starter_erbf_shrunk": None, "opposing_starter_bbf_shrunk": None, "opposing_starter_hbf_shrunk": None, "opposing_starter_bf_per_start_shrunk": None}
    return {
        "opposing_starter_erbf_shrunk": shrunk_rate(state["er"], state["bf"], league_prior["erbf"], rate_prior_bf),
...
def nb2_dispersion(y, mu):
    ...
    if denominator <= 0:
        raise SystemExit("V46_NB2_DENOMINATOR_INVALID")
...
def build_season_rows(root, batter_root, season, contract):
    ...
    if canonical.get("schemaVersion") != BASE_SCHEMA:
        raise SystemExit(f"V46_BASE_SCHEMA_INVALID:{season}")
    if batter_history.get("schemaVersion") != BATTER_SCHEMA:
        raise SystemExit(f"V46_BATTER_SCHEMA_INVALID:{season}")
        ...
        for raw in games:
            ...
            for side, lineup, opposing_pitcher_id in (("home", home_lineup, away_probable), ("away", away_lineup, home_probable)):
                for lineup_slot, batter_id in enumerate(lineup, 1):
                    ...
                    batter_only_mu = float(features["batter_rbipa_shrunk"]) * float(slot_pa) if finite(features["batter_rbipa_shrunk"]) and finite(slot_pa) else None
                    rows.append({
                        "season": season,
...
def evaluate(frame, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines):
    ...
    y = frame["rbi"].to_numpy(dtype=float)
    ...
    return {
        "rows": int(len(frame)),
        "observedMeanRbi": observed_mean,
        "meanModelRbi": model_mean,
        "meanBatterOnlyRbi": batter_mean,
        "trainingConstantMeanRbi": float(train_mean),
...
def main():
    ...
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V46_CONTRACT_SCHEMA_INVALID")
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit("V46_EMPTY_PARTITION")
    ...
    report = {
        ...
        "parent": {
            ...
            "priorBatterTotalBasesClosure": contract["parentEvidence"]["priorBatterTotalBasesClosure"],
        },

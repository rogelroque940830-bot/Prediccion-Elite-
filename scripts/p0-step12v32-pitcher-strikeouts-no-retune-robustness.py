...
REPORT_SCHEMA = 'courtedge-p0-step12v32-pitcher-strikeouts-no-retune-robustness.v1'
...
def load_v31_module(path):
    spec = importlib.util.spec_from_file_location('courtedge_v31_pitcher_strikeouts', path)
    if spec is None or spec.loader is None:
        raise SystemExit('V32_V31_MODULE_LOAD_FAILED')
...
def assert_close(label, actual, expected, tolerance):
    if not math.isfinite(float(actual)) or not math.isfinite(float(expected)):
        raise SystemExit(f'V32_PARITY_NONFINITE:{label}:{actual}:{expected}')
    if abs(float(actual) - float(expected)) > tolerance:
        raise SystemExit(f'V32_V31_PARITY_MISMATCH:{label}:{actual}:{expected}:tol={tolerance}')
...
def bootstrap_date_means(frame, metric_arrays, replicates, seed, confidence):
    ...
    if len(daily) < 2:
        raise SystemExit('V32_BOOTSTRAP_TOO_FEW_DATES')
...
def main():
    ...
    parser.add_argument('--v31-contract', required=True)
    parser.add_argument('--v31-script', required=True)
    parser.add_argument('--v31-report', required=True)
    parser.add_argument('--out', required=True)
    args = parser.parse_args()

    contract = load(args.contract)
    if contract.get('schemaVersion') != 'courtedge-p0-step12v32-pitcher-strikeouts-no-retune-robustness-contract.v1':
        raise SystemExit('V32_CONTRACT_SCHEMA_INVALID')
    v31_contract = load(args.v31_contract)
    v31_report = load(args.v31_report)
    if v31_report.get('classification') != contract['parentEvidence']['v31ClassificationRequired']:
        raise SystemExit('V32_V31_PARENT_CLASSIFICATION_INVALID')
    if not v31_report.get('candidateRubricPassed'):
        raise SystemExit('V32_V31_PARENT_GATE_NOT_PASSED')

    v31 = load_v31_module(args.v31_script)
    seasons = [
        v31_contract['dataBoundary']['modelFitSeason'],
        v31_contract['dataBoundary']['validationSeason'],
        *v31_contract['dataBoundary']['retrospectiveEvaluationSeasons'],
    ]
    records = []
    custody = {}
    for season in seasons:
        season_rows, season_custody = v31.build_season_rows(args.root, season, v31_contract)
        records.extend(season_rows)
        custody[season] = season_custody

    frame = pd.DataFrame.from_records(records)
    feature_names = tuple(v31_contract['features']['exactly'])
    train_season = v31_contract['dataBoundary']['modelFitSeason']
    validation_season = v31_contract['dataBoundary']['validationSeason']
    evaluation_seasons = v31_contract['dataBoundary']['retrospectiveEvaluationSeasons']
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit('V32_EMPTY_PARTITION')
    ...
    dispersion = v31.nb2_dispersion(y_train, train_mu)
    train_mean = float(np.mean(y_train))
    lines = [float(value) for value in cfg['fixedHalfRunLines']]
    constant_line_probs = {line: float(np.mean(y_train > line)) for line in lines}

    validation_metrics = v31.evaluate(
        validation, feature_names, imputer, scaler, model, dispersion, train_mean, constant_line_probs, lines
    )
    evaluation_metrics = v31.evaluate(
        evaluation, feature_names, imputer, scaler, model, dispersion, train_mean, constant_line_probs, lines
    )

    tolerance = float(contract['replayParity']['absoluteTolerance'])
    required_metrics = contract['replayParity']['requiredMetrics']
    for metric in required_metrics:
        assert_close(f'validation2023.{metric}', validation_metrics[metric], v31_report['validation2023'][metric], tolerance)
        assert_close(
            f'evaluation2024_2026Ytd.{metric}',
            evaluation_metrics[metric],
            v31_report['evaluation2024_2026Ytd'][metric],
            tolerance,
        )
    ...
    for season in contract['seasonStabilityGate']['partitions']:
        ...
        metrics = v31.evaluate(
            part, feature_names, imputer, scaler, model, dispersion, train_mean, constant_line_probs, lines
        )
    ...
    y_eval = eval_frame['strikeouts'].to_numpy(dtype=float)
    ...
    for line in lines:
        ...
        model_prob = np.asarray([v31.over_probability(mu, dispersion, line) for mu in model_mu], dtype=float)
        pitcher_prob = np.asarray([v31.over_probability(mu, dispersion, line) for mu in pitcher_mu], dtype=float)
    ...
    bootstrap_checks = {
        name: values['ciLower'] > 0.0 for name, values in bootstrap['metrics'].items()
    }
    ...
    report = {
        ...
        'parent': {
            'v31Classification': v31_report['classification'],
            'v31CandidateRubricPassed': v31_report['candidateRubricPassed'],
            'v31WorkflowRunId': contract['parentEvidence']['v31WorkflowRunId'],
            'v31ArtifactId': contract['parentEvidence']['v31ArtifactId'],
        },
        'modelBoundary': {
            'unchangedV31ImplementationReplayed': True,

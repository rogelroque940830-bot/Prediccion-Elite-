...
REPORT_SCHEMA = "courtedge-p0-step12v44-batter-total-bases-baseline.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v44-batter-total-bases-baseline-contract.v1"
...
def parse_starter(raw):
    ...
    for key, source in mapping.items():
        ...
        if not finite(value) or float(value) < 0:
            raise SystemExit(f"V44_INVALID_STARTER_LINE:{pitcher_id}:{source}:{value}")
...
def empty_pitcher_state():
    return {"starts": 0, "bf": 0.0, "h": 0.0, "hr": 0.0, "k": 0.0}
...
def add_batter_line(state, line, recent_window):
    ...
    if pa <= 0:
        return
    hits = int(line["hits"])
    total_bases = int(line["totalBases"])
    xbh = int(line["doubles"]) + int(line["triples"]) + int(line["homeRuns"])
    strikeouts = int(line["strikeOuts"])
    state["games"] += 1
    state["pa"] += pa
    state["h"] += hits
    state["tb"] += total_bases
    state["xbh"] += xbh
    state["k"] += strikeouts
    state["recent"].append({"tb": total_bases, "pa": pa})
...
def batter_features(state, league_prior, prior_pa):
    recent = state["recent"]
    features = {
        "batter_recent10_tb_per_game": (
            float(np.mean([row["tb"] for row in recent])) if recent else None
        ),
    ...
    features.update(
        {
            ...
            "batter_xbhpa_shrunk": shrunk_rate(
                state["xbh"], state["pa"], league_prior["xbhpa"], prior_pa
            ),
...
def opposing_starter_features(state, league_prior, rate_prior_bf, workload_prior_starts):
    if league_prior is None:
        return {
            ...
            "opposing_starter_hrbf_shrunk": None,
    ...
    return {
        ...
        "opposing_starter_hrbf_shrunk": shrunk_rate(
            state["hr"], state["bf"], league_prior["hrbf"], rate_prior_bf
        ),
...
def nb2_dispersion(y, mu):
    ...
    if denominator <= 0:
        raise SystemExit("V44_NB2_DENOMINATOR_INVALID")
...
def build_season_rows(root, batter_root, season, contract):
    ...
    if canonical.get("schemaVersion") != BASE_SCHEMA:
        raise SystemExit(f"V44_BASE_SCHEMA_INVALID:{season}")
    if batter_history.get("schemaVersion") != BATTER_SCHEMA:
        raise SystemExit(f"V44_BATTER_SCHEMA_INVALID:{season}")
    ...
    for official_date in sorted(by_date):
        ...
        for raw in games:
            ...
            for side, lineup, opposing_pitcher_id in (
                ...
                for lineup_slot, batter_id in enumerate(lineup, 1):
                    ...
                    batter_only_mu = (
                        float(features["batter_tbpa_shrunk"]) * float(slot_pa)
                        if finite(features["batter_tbpa_shrunk"]) and finite(slot_pa)
                        else None
                    )
                    rows.append(
                        {
                            ...
                            "totalBases": int(line["totalBases"]),
        ...
        # Strict boundary: every target on an official date is scored before any same-date update.
        for raw in games:
            game_pk = int(raw["gamePk"])
            batter_game = batter_game_map.get(game_pk)
            starter_game = starter_map.get(game_pk)
...
def evaluate(frame, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines):
    ...
    y = frame["totalBases"].to_numpy(dtype=float)
    ...
    return {
        "rows": int(len(frame)),
        "observedMeanTotalBases": observed_mean,
        "meanModelTotalBases": model_mean,
        "meanBatterOnlyTotalBases": batter_mean,
        "trainingConstantMeanTotalBases": float(train_mean),
...
def main():
    ...
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V44_CONTRACT_SCHEMA_INVALID")
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit("V44_EMPTY_PARTITION")

    imputer = SimpleImputer(strategy="median")
    scaler = StandardScaler()
    X_train = scaler.fit_transform(imputer.fit_transform(train[list(features)]))
    y_train = train["totalBases"].to_numpy(dtype=float)
    ...
    lines = [float(value) for value in model_cfg["fixedHalfTotalBasesLines"]]
    line_probs = {line: float(np.mean(y_train > line)) for line in lines}

    validation_metrics = evaluate(
        validation, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines
    )
    evaluation_metrics = evaluate(
        evaluation, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines
    )
    by_season = {
        season: evaluate(
    ...
    classification = (
        ...
        if passed
        else contract["candidateRubric"]["failureClassification"]
    )
    ...
    report = {
        ...
        "parent": {
            ...
            "priorBatterHitsClosure": contract["parentEvidence"]["priorBatterHitsClosure"],
        },
        "data": {
        ...
        "model": {
            "providerMarketKey": "batter_total_bases",
            "canonicalResearchMarketType": "BATTER_TOTAL_BASES",
            "trainingSeason": "2022",
            "trainingRows": int(len(train)),
            "trainingMeanTotalBases": train_mean,
            "poissonAlpha": float(model_cfg["poissonAlpha"]),
            "maxIter": int(model_cfg["maxIter"]),
            "nb2Dispersion": float(dispersion),
            "fixedHalfTotalBasesLines": lines,
            ...
            "coefficientsDescriptiveNotSelectionInput": {
                features[index]: float(model.coef_[index]) for index in range(len(features))
            },
        },
        "baselines": {
            "constantMeanTotalBases": train_mean,
        ...
        "marketBoundary": {
            ...
            "historicalBatterTotalBasesPricesUsed": False,

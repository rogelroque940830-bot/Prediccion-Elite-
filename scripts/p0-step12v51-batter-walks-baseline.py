...
REPORT_SCHEMA = "courtedge-p0-step12v51-batter-walks-baseline.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v51-batter-walks-baseline-contract.v1"
...
def parse_starter(raw):
    ...
    if pitcher_id is None:
        return None
    mapping = {"bf": "battersFaced", "bb": "baseOnBalls", "k": "strikeOuts"}
    values = {"pitcherId": pitcher_id}
    for key, source in mapping.items():
        value = raw.get(source, 0)
        if not finite(value) or float(value) < 0:
            raise SystemExit(f"V51_INVALID_STARTER_LINE:{pitcher_id}:{source}:{value}")
        values[key] = float(value)
    return values


def empty_batter_state():
    return {"games": 0, "pa": 0.0, "bb": 0.0, "k": 0.0, "h": 0.0, "recent": []}


def empty_pitcher_state():
    return {"starts": 0, "bf": 0.0, "bb": 0.0, "k": 0.0}
...
def add_batter_line(state, line, recent_window):
    ...
    hits = int(line["hits"])
    if min(walks, strikeouts, hits) < 0:
        raise SystemExit("V51_NEGATIVE_BATTER_STAT")
    state["games"] += 1
    state["pa"] += pa
    state["bb"] += walks
    state["k"] += strikeouts
    state["h"] += hits
    state["recent"].append({"bb": walks, "pa": pa})
...
def batter_features(state, league_prior, prior_pa):
    recent = state["recent"]
    features = {
        "batter_recent10_bb_per_game": float(np.mean([row["bb"] for row in recent])) if recent else None,
        "batter_recent10_pa_per_game": float(np.mean([row["pa"] for row in recent])) if recent else None,
    }
    if league_prior is None:
        features.update({"batter_bbpa_shrunk": None, "batter_kpa_shrunk": None, "batter_hpa_shrunk": None})
        return features
    features.update(
        {
...
def opposing_starter_features(state, league_prior, rate_prior_bf, workload_prior_starts):
    ...
    return {
        "opposing_starter_bbbf_shrunk": shrunk_rate(state["bb"], state["bf"], league_prior["bbbf"], rate_prior_bf),
...
def nb2_dispersion(y, mu):
    ...
    if denominator <= 0:
        raise SystemExit("V51_NB2_DENOMINATOR_INVALID")
...
def build_season_rows(root, batter_root, season, contract):
    ...
    if canonical.get("schemaVersion") != BASE_SCHEMA:
        raise SystemExit(f"V51_BASE_SCHEMA_INVALID:{season}")
    if batter_history.get("schemaVersion") != BATTER_SCHEMA:
        raise SystemExit(f"V51_BATTER_SCHEMA_INVALID:{season}")
        ...
        for raw in games:
            ...
            for side, lineup, opposing_pitcher_id in (
                ...
                for lineup_slot, batter_id in enumerate(lineup, 1):
                    ...
                    batter_only_mu = (
                        float(features["batter_bbpa_shrunk"]) * float(slot_pa)
                        if finite(features["batter_bbpa_shrunk"]) and finite(slot_pa)
                        else None
                    )
                    rows.append(
                        {
...
def evaluate(frame, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines):
    ...
    return {
        "rows": int(len(frame)),
        "observedMeanWalks": observed_mean,
        "meanModelWalks": model_mean,
        "meanBatterOnlyWalks": batter_mean,
...
def main():
    ...
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V51_CONTRACT_SCHEMA_INVALID")
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit("V51_EMPTY_PARTITION")
    ...
    lines = [float(value) for value in model_cfg["fixedHalfWalkLines"]]
    ...
    report = {
        ...
        "parent": {
            ...
            "priorBatterStrikeoutsClosure": contract["parentEvidence"]["priorBatterStrikeoutsClosure"],
        },
        "data": {
        ...
        "marketBoundary": {
            "providerMarketKey": "batter_walks",
            "repositoryRegistryFamily": "BATTER_PROP",
            "hardRockEvidenceStatus": "NOT_VERIFIED",
            "historicalBatterWalkPricesUsed": False,

...
REPORT_SCHEMA = "courtedge-p0-step12v50-batter-strikeouts-baseline.v1"
CONTRACT_SCHEMA = "courtedge-p0-step12v50-batter-strikeouts-baseline-contract.v1"
...
def parse_starter(raw):
    ...
    for key, source in mapping.items():
        ...
        if not finite(value) or float(value) < 0:
            raise SystemExit(f"V50_INVALID_STARTER_LINE:{pitcher_id}:{source}:{value}")
        values[key] = float(value)
    return values


def empty_batter_state():
    return {"games": 0, "pa": 0.0, "k": 0.0, "h": 0.0, "bb": 0.0, "recent": []}


def empty_pitcher_state():
    return {"starts": 0, "bf": 0.0, "k": 0.0, "bb": 0.0}
...
def add_batter_line(state, line, recent_window):
    ...
    if min(strikeouts, hits, walks) < 0:
        raise SystemExit("V50_NEGATIVE_BATTER_STAT")
    state["games"] += 1
    state["pa"] += pa
    state["k"] += strikeouts
    state["h"] += hits
    state["bb"] += walks
    state["recent"].append({"k": strikeouts, "pa": pa})
...
def shrunk_rate(numerator, denominator, league_rate, prior_weight):
    return (float(numerator) + float(prior_weight) * float(league_rate)) / (
        float(denominator) + float(prior_weight)
    )
...
def batter_features(state, league_prior, prior_pa):
    recent = state["recent"]
    features = {
        "batter_recent10_k_per_game": float(np.mean([row["k"] for row in recent])) if recent else None,
    ...
    features.update(
        {
            "batter_kpa_shrunk": shrunk_rate(state["k"], state["pa"], league_prior["kpa"], prior_pa),
...
def opposing_starter_features(state, league_prior, rate_prior_bf, workload_prior_starts):
    if league_prior is None:
        return {
            ...
            "opposing_starter_bbf_shrunk": None,
            "opposing_starter_bf_per_start_shrunk": None,
        }
    return {
        "opposing_starter_kbf_shrunk": shrunk_rate(state["k"], state["bf"], league_prior["kbf"], rate_prior_bf),
        ...
        "opposing_starter_bf_per_start_shrunk": shrunk_mean(
            state["bf"], state["starts"], league_prior["bfPerStart"], workload_prior_starts
        ),
    }
...
def nb2_dispersion(y, mu):
    ...
    if denominator <= 0:
        raise SystemExit("V50_NB2_DENOMINATOR_INVALID")
...
def build_season_rows(root, batter_root, season, contract):
    ...
    if canonical.get("schemaVersion") != BASE_SCHEMA:
        raise SystemExit(f"V50_BASE_SCHEMA_INVALID:{season}")
    if batter_history.get("schemaVersion") != BATTER_SCHEMA:
        raise SystemExit(f"V50_BATTER_SCHEMA_INVALID:{season}")
    ...
    for official_date in sorted(by_date):
        ...
        for raw in games:
            ...
            for side, lineup, opposing_pitcher_id in (
                ...
                for lineup_slot, batter_id in enumerate(lineup, 1):
                    ...
                    batter_only_mu = (
                        float(features["batter_kpa_shrunk"]) * float(slot_pa)
                        if finite(features["batter_kpa_shrunk"]) and finite(slot_pa)
                        else None
                    )
                    rows.append(
                        {
        ...
        # Strict same-date batch update: nothing scored on this date may use same-date outcomes.
        for raw in games:
            game_pk = int(raw["gamePk"])
            batter_game = batter_game_map.get(game_pk)
            starter_game = starter_map.get(game_pk)
...
def evaluate(frame, features, imputer, scaler, model, dispersion, train_mean, line_probs, lines):
    ...
    return {
        "rows": int(len(frame)),
        "observedMeanStrikeouts": observed_mean,
        "meanModelStrikeouts": model_mean,
        "meanBatterOnlyStrikeouts": batter_mean,
...
def volume_diagnostics(frame, custody):
    ...
    return {
        ...
        "eligibleStartingBatterOutcomes": int(eligible),
        "frozenPregameStartingBatterSlots": int(frozen_slots),
        "eligibilityShare": eligible / frozen_slots if frozen_slots else 0.0,
        "meanEligibleStartingBattersPerSlateDay": float(np.mean(list(counts.values()))) if counts else 0.0,
        "medianEligibleStartingBattersPerSlateDay": float(np.median(list(counts.values()))) if counts else 0.0,
...
def main():
    ...
    if contract.get("schemaVersion") != CONTRACT_SCHEMA:
        raise SystemExit("V50_CONTRACT_SCHEMA_INVALID")
    ...
    if min(len(train), len(validation), len(evaluation)) <= 0:
        raise SystemExit("V50_EMPTY_PARTITION")
    ...
    lines = [float(value) for value in model_cfg["fixedHalfStrikeoutLines"]]
    ...
    checks = {
        ...
        "validationDevianceBeatsBatterOnly": validation_metrics["modelVsBatterOnlyDevianceImprovement"] > 0.0,
        "validationBrierBeatsConstant": validation_metrics["modelVsConstantAverageBrierImprovement"] > 0.0,
        "validationBrierBeatsBatterOnly": validation_metrics["modelVsBatterOnlyAverageBrierImprovement"] > 0.0,
        "evaluationDevianceBeatsConstant": evaluation_metrics["modelVsConstantDevianceImprovement"] > 0.0,
        "evaluationDevianceBeatsBatterOnly": evaluation_metrics["modelVsBatterOnlyDevianceImprovement"] > 0.0,
        "evaluationBrierBeatsConstant": evaluation_metrics["modelVsConstantAverageBrierImprovement"] > 0.0,
        "evaluationBrierBeatsBatterOnly": evaluation_metrics["modelVsBatterOnlyAverageBrierImprovement"] > 0.0,
    }
    ...
    report = {
        ...
        "parent": {
            "mainBaseSha": contract["parentEvidence"]["mainBaseSha"],
            "v41Classification": contract["parentEvidence"]["v41CertificationRequired"],
            "priorBatterRunsScoredClosure": contract["parentEvidence"]["priorBatterRunsScoredClosure"],
        },
        "data": {
        ...
        "baselines": {
            ...
            "batterOnlyMissingPredictionFallback": train_mean,
            "sharedTrainingOnlyNb2DispersionForProbabilityConversion": float(dispersion),
        },
        ...
        "marketBoundary": {
            "providerMarketKey": "batter_strikeouts",
            "repositoryRegistryFamily": "BATTER_PROP",
            "historicalBatterStrikeoutPricesUsed": False,

...
test("-126 total preserves arithmetic and the live ledger line is a valid 4.0", () => {
  const pick = record("rangers-edge-outlier");
  assert.equal(pick.classification, "REVIEW");
  assert.ok(pick.issues.some((entry) => entry.code === "EDGE_OUTLIER"));
...
test("-4 proves invalid American odds were accepted upstream while the line remains valid 6.0", () => {

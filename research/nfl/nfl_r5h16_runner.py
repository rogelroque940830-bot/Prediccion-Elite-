...
def team_concentration_from_game_id(df: pd.DataFrame, selected, source: pd.DataFrame) -> pd.DataFrame:
    ...
    # without adding any new data source or touching selection/outcome logic.
    ...
    if parts.shape[1] != 3 or parts[1].isna().any() or parts[2].isna().any():
        raise RuntimeError("R5H16 could not recover team labels from canonical nflverse game_id")
